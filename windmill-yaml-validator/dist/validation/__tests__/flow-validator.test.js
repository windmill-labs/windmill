...
const flow_validator_1 = require("../flow-validator");
...
describe('FlowValidator', () => {
    ...
    beforeEach(() => {
        validator = new flow_validator_1.FlowValidator();
    });
    describe('constructor', () => {
        it('should create a validator instance', () => {
            expect(validator).toBeInstanceOf(flow_validator_1.FlowValidator);
        });
        it('should initialize without throwing', () => {
            expect(() => new flow_validator_1.FlowValidator()).not.toThrow();
        });
    });
    describe('validateFlow', () => {
        it('should throw error for non-string input', () => {
            expect(() => validator.validateFlow(null)).toThrow('Document must be a string');
            expect(() => validator.validateFlow(123)).toThrow('Document must be a string');
            expect(() => validator.validateFlow({})).toThrow('Document must be a string');
            expect(() => validator.validateFlow([])).toThrow('Document must be a string');
        });
        ...
        describe('invalid flows', () => {
            it('should return errors for missing summary from sample file', () => {
                ...
                expect(result.errors.some(error => error.instancePath === '' &&
                    error.keyword === 'required' &&
                    error.params?.missingProperty === 'summary')).toBe(true);
            });
            it('should return errors for invalid types from sample file', () => {
                const invalidFlow = readSample('invalid-wrong-types.yaml');
                const result = validator.validateFlow(invalidFlow);
                expect(result.errors.length).toBeGreaterThan(0);
                expect(result.errors.some(error => error.instancePath === '/summary' &&
                    error.keyword === 'type')).toBe(true);
            });
            it('should return errors for invalid language from sample file', () => {
                const invalidFlow = readSample('invalid-language.yaml');
                const result = validator.validateFlow(invalidFlow);
                expect(result.errors.length).toBeGreaterThan(0);
                expect(result.errors.some(error => error.instancePath === '/value/modules/0/value/language' &&
                    error.keyword === 'enum')).toBe(true);
            });
            ...
            it('should handle complex invalid flow with comprehensive error detection', () => {
                ...
                const forloopRequiredErrors = result.errors.filter(error => error.keyword === 'required' && error.message &&
                ...
                const branchRequiredErrors = result.errors.filter(error => error.keyword === 'required' && error.message &&
                ...
                const discriminatorErrors = result.errors.filter(error => error.keyword === 'discriminator');
            ...
            it('should handle deeply nested invalid structures with detailed error reporting', () => {
                ...
                const deepNestedErrors = result.errors.filter(error => error.instancePath.split('/').length > 6 // Deep nesting
                );
                expect(deepNestedErrors.length).toBeGreaterThan(0);
                // Check for transform-related errors
                const transformErrors = result.errors.filter(error => error.instancePath.includes('input_transforms') ||
                ...
                const nestedRequiredErrors = result.errors.filter(error => error.keyword === 'required' &&
                ...
                const nestedDiscriminatorErrors = result.errors.filter(error => error.keyword === 'discriminator');
            ...
            it('should provide specific error locations for complex validation failures', () => {
                ...
                const errorsWithPaths = result.errors.filter(error => error.instancePath && error.instancePath.length > 0);
                expect(errorsWithPaths.length).toBeGreaterThan(5);
                // Check that we can identify specific problematic modules
                const moduleSpecificErrors = result.errors.filter(error => error.instancePath.includes('/value/modules/'));
                expect(moduleSpecificErrors.length).toBeGreaterThan(0);
                // Verify error messages are descriptive
                const descriptiveErrors = result.errors.filter(error => error.message && error.message.length > 0);
            ...
            it('should handle all major flow control structures with validation errors', () => {
                ...
                const flowControlErrors = result.errors.filter(error => error.instancePath.includes('/modules/2/') || // forloop module
                ...
                const pathReferenceErrors = result.errors.filter(error => error.instancePath.includes('/modules/8/') || // script path module
                ...
                const missingIdErrors = result.errors.filter(error => error.keyword === 'required' && error.message &&
                    error.message.includes('id'));
                expect(missingIdErrors.length).toBeGreaterThan(0);
                // Should detect type mismatches at the flow level
                const flowLevelTypeErrors = result.errors.filter(error => error.instancePath.startsWith('/value/') &&
                    !error.instancePath.includes('/modules/') &&
                    error.keyword === 'type');
