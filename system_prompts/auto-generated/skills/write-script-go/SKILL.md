...
This only writes local files (it is **not** a deploy), but it re-resolves dependencies, which can bump unpinned versions (the same as deploying from the UI). So by default offer it and run it once the user agrees, rather than running it silently after every edit — unless the project's `AGENTS.md` opts into running metadata automatically (see the "Keeping metadata in sync" preference there). Either way YOU run the command, not the user. Dep bumps are expected and fine — pin versions in code if you need them fixed.
