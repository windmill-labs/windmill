<script lang="ts">
	...
	// Mirror the user-typed path into the autosaved Flow's own `path`. The Path
	// widget binds `$pathStore` (the popover input); the effect above seeds it
	// one-way from `flow.path`, and this one writes edits back — so `flow.path`
	// IS the typed target (like the script editor's `script.path`). Without it,
	// the friendly auto-name on `/flows/add` and any in-place rename never reach
	// the autosaved Flow, so the home-list draft row keeps showing the
	// autogenerated `u/{user}/draft_{uuid}` slot. The deployed/original path is
	// `initialPath` (the URL), not `flow.path`. Deploy clears the whole draft.
	$effect(() => {
		const typed = $pathStore
		const flow = flowStore.val as Flow | undefined
		if (!flow) return
		untrack(() => {
			if (typed != undefined && typed !== '' && flow.path !== typed) {
				flow.path = typed
			}
		})
	})
