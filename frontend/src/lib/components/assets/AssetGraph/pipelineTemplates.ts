...
// ========================== Ingestion templates ==========================
// Multi-script starters for the EL (extract-load) side of a pipeline. Unlike
// the per-output-kind skeletons above (one body, shape picked column by
// column), an ingestion template is an opinionated, runnable set of scripts:
// an entry node that pulls from an external source and a managed
// `-- materialize` loader that lands the batch in the lake.
//
// Why the polyglot templates are TWO scripts (entry + DuckDB loader) instead
// of one script writing the lake directly:
//   - `// materialize` is DuckDB-only (deploy rejects it on other languages),
//     and the SDK materialize helpers (`upsert_partition` & co) build their
//     SQL internally, so the asset parser can't see the write — a single
//     Python/TS node would deploy with NO output edge in the graph.
//   - splitting E (network-bound, any language) from L (managed DuckDB
//     materialize) keeps the heavy load path vectorized and gives the loader
//     the full engine treatment: snapshots, partition grid, backfill,
//     `// data_test`, idempotent strategies.
// The seam between the two is a raw landing object in workspace storage; the
// loader's `-- on s3:///…` trigger re-runs it whenever a new batch lands.
export type IngestionTemplateScript = {
	// Appended to the user-picked base path ('' = the entry script itself).
	suffix: string
	language: ScriptLang
	content: string
	// Seeded output for the draft overlay while the draft is not the open
	// buffer (live inference only runs for the active draft). Loaders don't
	// need it — their `-- materialize` annotation carries the write.
	output?: { kind: AssetKind; path: string }
}
...
export type IngestionTemplateMeta = {
	...
	label: string
	description: string
	generate: (ctx: { folder: string; base: string }) => IngestionTemplateScript[]
}

// Canonical landing-object paths for a template's staging file. The SDK
// object form (`{s3: <key>}` / `S3Object(s3=<key>)`) takes the bare key; the
// annotation + DuckDB URI form is `s3:///<key>` (triple slash = default
// storage). Both canonicalize to `/<key>`, which is what connects the entry's
// write edge to the loader's trigger + read.
function landingKey(folder: string, base: string): string {
	return `pipelines/${folder}/landing/${base}.jsonl`
}

const PIPELINE_DOCS_URL = 'https://www.windmill.dev/docs/core_concepts/pipelines'

function postgresIncrementalSql(base: string): string {
	return [
		`-- pipeline`,
		`-- on schedule`,
		`-- materialize ducklake://main/${base} key=id`,
		`-- Strategy: merge (upsert by key); add \`history\` after key=id for SCD2 versions`,
		``,
		`-- Incremental pull without cursor state: re-read only the rows touched in`,
		`-- the lookback window — merge-by-key makes re-pulling the same rows`,
		`-- idempotent, so the window just has to be wider than the schedule gap.`,
		`-- Widen (or drop) the filter for a full backfill run.`,
		`-- More: ${PIPELINE_DOCS_URL}`,
		`ATTACH 'datatable://main' AS pg;`,
		``,
		`SELECT *`,
		`FROM pg.source_table -- point at your operational table`,
...
function restCursorEntryTs(folder: string, base: string): string {
	const key = landingKey(folder, base)
	return [
		`// pipeline`,
		`// on schedule`,
		``,
		`// Plain incremental REST ingestion: fetch pages, keep rows past the cursor,`,
		`// land the raw batch in object storage. The \`${base}_load\` script appends`,
		`// each batch to the lake. The cursor persists per-script — reset it (Details`,
		`// pane → State, or \`wmill.setState(null)\`) to re-ingest from scratch.`,
		`// More: ${PIPELINE_DOCS_URL}`,
		``,
...
function dltRestEntryPy(folder: string, base: string): string {
	const key = landingKey(folder, base)
	return [
		`# pipeline`,
		`# on schedule`,
		``,
		`# dlt-powered incremental REST ingestion: dlt's RESTClient handles pagination`,
		`# (auto-detects header-link / cursor / offset styles; explicit paginators and`,
		`# auth classes for everything else), Windmill state stores the cursor, and the`,
		`# raw batch lands in object storage for the \`${base}_load\` script to merge`,
		`# into the lake. Reach for dlt when you want its source ecosystem — for a`,
		`# plain REST endpoint, the fetch-loop template does the same with less.`,
		`# Reset the state (Details pane → State) to re-ingest from scratch.`,
		`# More: ${PIPELINE_DOCS_URL}`,
		``,
		`import json`,
		``,
		...
		``,
		`    wmill.write_s3_file(`,
		`        wmill.S3Object(s3=${JSON.stringify(key)}),`,
...
// The managed loader half of a two-script template: triggered by the landing
// object, materializes it into the lake with the given strategy line.
function landingLoaderSql(folder: string, base: string, strategy: 'merge' | 'append'): string {
	const uri = `s3:///${landingKey(folder, base)}`
	const strategyLine =
		strategy === 'merge'
			? [
					`-- materialize ducklake://main/${base} key=id`,
			...
			: [
					`-- materialize ducklake://main/${base} append`,
	...
	return [
		...
		...strategyLine,
		``,
		`SELECT * FROM read_json_auto('${uri}');`,
		``
	].join('\n')
}

// Menu order = recommended order: the pure-DuckDB pull first (no per-row
// scripting in the load path at all), then the plain fetch loop, then dlt for
// when its connector ecosystem is worth the Python extract layer.
export const INGESTION_TEMPLATES: IngestionTemplateMeta[] = [
	{
		id: 'postgres_incremental',
		label: 'Postgres → lake',
		description: 'Incremental pull in pure DuckDB: updated_at window, idempotent merge by key',
		generate: ({ base }) => [
			{ suffix: '', language: 'duckdb', content: postgresIncrementalSql(base) }
		]
	},
	{
		id: 'rest_api_cursor',
		label: 'REST API → lake',
		description:
			'TypeScript fetch pagination + cursor in state → landing file → append-materialize',
		generate: ({ folder, base }) => [
			{
				suffix: '',
				language: 'bun',
				content: restCursorEntryTs(folder, base),
				output: { kind: 's3object', path: `/${landingKey(folder, base)}` }
			},
			{
				suffix: '_load',
				language: 'duckdb',
				content: landingLoaderSql(folder, base, 'append')
			}
		]
	},
	{
		id: 'dlt_rest_api',
		label: 'REST API → lake (dlt)',
		description: 'dlt RESTClient pagination + cursor in state → landing file → merge-materialize',
		generate: ({ folder, base }) => [
			{
				suffix: '',
				language: 'python3',
				content: dltRestEntryPy(folder, base),
				output: { kind: 's3object', path: `/${landingKey(folder, base)}` }
			},
			{
				suffix: '_load',
				language: 'duckdb',
				content: landingLoaderSql(folder, base, 'merge')
			}
		]
	}
]
