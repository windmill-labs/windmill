<script lang="ts">
    ...
    export let icon: typeof SvelteComponent = InfoIcon; // Default to InfoIcon
    export let customSize: string = '100%'; // Default size as 100%

    const plugins = [gfmPlugin()];
</script>

<Popover notClickable {placement} class={wrapperClass} style="transform: scale({parseFloat(customSize) / 100});">
    <div class="inline-flex w-3 mx-0.5 {light ? 'text-tertiary-inverse' : 'text-tertiary'} {$$props.class} relative">
        <svelte:component this={icon} class="{small ? 'bottom-0' : '-bottom-0.5'} absolute" size={small ? 12 : 14} />
    </div>
