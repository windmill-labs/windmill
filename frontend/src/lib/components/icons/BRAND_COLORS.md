...
| `AbstractApiIcon` | `abstractapi` | fixed | #20E492 | #20E492 | **1.62** | 12.47 | abstractapi.com's own favicon (65390de624cb65770560dda5_FAV.png) and site CSS |
...
| `AcumbamailIcon` | `acumbamail` | fixed | #E62F71 | #E62F71 | 8.83 | 8.86 | Acumbamail's own logo SVG, /static/public-theme/img/logos/acumbamail-color-home-logo.svg on acumbamail.com |
...
| `ApifyIcon` | `apify` | fixed | #246DFF | #246DFF | 4.32 | 12.47 | apify.com/favicon.svg |
...
| `BaremetricsIcon` | `baremetrics` | fixed | #6078FF | #6078FF | 3.61 | 3.34 | baremetrics.com/favicon.ico |
...
| `ClaudeIcon` | — | fixed | #D97757 | #D97757 | 3.02 | 12.47 | https://claude.ai/favicon.svg (Anthropic's own asset); the glyph is knocked out in white on the tile |
| `ClearbitIcon` | `clearbit` | fixed | #4DB1FD | #4DB1FD | 20.32 | 10.83 | clearbit.com/favicon-192.png — Clearbit publishes no brand page |
...
| `DatabricksIcon` | `databricks` | fixed | #FF3621 | #FF3621 | 3.50 | 3.45 | databricks.com favicon and Databricks' press kit (brandfolder.com/p/gjdn403461/press-kit) |
...
| `FormstackIcon` | `formstack` | fixed | #21B573 | #21B573 | 2.56 | 4.70 | the Formstack logo SVG served from formstack.com |
...
| `GmailIcon` | `gmail` | mixed | #4285F4 | #4285F4 | 5.61 | 7.30 | Google's own product icon asset (gstatic.com/images/branding/product/2x/gmail_2020q4_48dp.png), whose five fills match exactly |
...
| `GroqIcon` | `groqai`, `groq` | fixed | #F43E01 | #F43E01 | 3.67 | 12.47 | https://groq.com/favicon.svg (mark copied verbatim; no per-theme variant published) |
...
| `LinodeIcon` | `linode` | fixed | #004711 | #004711 | 10.64 | 4.41 | Linode's own logo-text.svg in linode/manager (packages/manager/src/assets/logo) |
| `LumaAiIcon` | `lumaai` | pair | #000000 | #FFFFFF | 20.32 | 12.47 | lumalabs.ai (favicon-black.ico on light, favicon-white.ico on dark) |
| `MSSqlServerIcon` | — | fixed | #0094F0 | #0094F0 | 10.54 | 12.47 | learn.microsoft.com/en-us/azure/architecture/icons — dominant blue of Microsoft's own SQL Server icon; the paler and indigo gradient stops here have no first-party source |
...
| `MailerLiteIcon` | `mailerlite` | fixed | #21C16C | #21C16C | 2.28 | 5.29 | MailerLite's own logo pack, linked from mailerlite.com/brand-assets |
...
| `PushoverIcon` | `pushover` | fixed | #249DF1 | #249DF1 | 2.83 | 12.47 | — |
| `QoveryIcon` | `qovery` | fixed | #642DFF | #642DFF | 6.05 | 2.00 | qovery.com/logos/qovery-logo-black.svg |
| `QuickbooksIcon` | `quickbooks` | fixed | #2CA01C | #2CA01C | 3.30 | 3.65 | Intuit's own QuickBooks logotype SVG on developer.intuit.com (same value as the `quickbooks` brand token in intuit.com's design-system CSS) |
...
| `RecraftIcon` | `recraft` | fixed | #000000 | #000000 | 20.32 | 12.47 | recraft.ai's own favicon (https://www.recraft.ai/) |
...
| `RssIcon` | `rss` | fixed | #FFA500 | #FFA500 | 1.91 | 12.47 | Mozilla's feed icon guidelines (mozilla.org/en-US/foundation/feed-icon-guidelines/), which mandate orange (or an inverted/greyscale variant) without fixing a hex; the mark must never be rotated or flipped |
...
| `ServiceNowIcon` | `servicenow` | fixed | #62D84E | #62D84E | 1.77 | 6.80 | — |
...
| `SupabaseIcon` | `supabase` | fixed | #3ECF8E | #3ECF8E | 3.75 | 6.25 | supabase.com/favicon/favicon.svg |
...
| `TogetherAiIcon` | `togetherai` | fixed | #EF2CC1 | #EF2CC1 | 3.50 | 6.46 | together.ai's own brand page (https://www.together.ai/brand), which paints its logo with these three; they are the brand's foundational palette, no per-theme variant |
...
| `TwitchIcon` | `twitch` | fixed | #9146FF | #9146FF | 4.49 | 2.69 | — |
...
| `WebflowIcon` | `webflow` | fixed | #146EF5 | #146EF5 | 4.44 | 2.72 | — |
...
| `ZammadIcon` | `zammad` | fixed | #CA2317 | #CA2317 | 7.59 | 9.53 | — |
...
- **ApifyIcon** — Fixed tricolour mark, no per-theme variant published.
...
- **BaremetricsIcon** — Their site header logo ships the same mark at #5386FF — first-party assets disagree, so neither is a safe "correction" of the other.
...
- **BunIcon** — Fixed full-colour mark: every fill matches the published SVG byte for byte, so it takes no per-theme variant.
...
- **ClearbitIcon** — Fixed full-colour mark; #4DB1FD / #DEF2FE are its lighter panels.
...
- **GmailIcon** — Google's branding guidelines forbid modifying or recolouring the mark.
...
- **HoneybadgerIcon** — Single-colour bolt mark, no per-theme variant published.
...
- **LinodeIcon** — The third path is deliberately unfilled: it inherits currentColor as the keyline does in that source asset.
...
- **RecraftIcon** — Recraft serves that same black-plate icon to prefers-color-scheme light and dark — not a theme pair.
...
- **SupabaseIcon** — supabase.com/brand-assets forbids modifying or recolouring the mark.
...
- brand icons: **304**, of which **296** carry a recorded source
