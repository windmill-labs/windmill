<script lang="ts">
	interface Props {
		height?: string
		width?: string
		size?: number
	}

	let { height = '24px', width = '24px', size = 16 }: Props = $props()

	const derivedWidth = $derived(size ? size : width)
	const derivedHeight = $derived(size ? size : height)
</script>

<!-- #42B883 / #35495E per vuejs/art logo.svg. Vue's dark-background variant is separate outlined artwork rather than a recolour, so the two-tone mark is used in both themes. -->
<svg
	width={derivedWidth}
	height={derivedHeight}
	viewBox="-1.455 -1.455 34.909 34.909"
	xmlns="http://www.w3.org/2000/svg"
	version="1.1"
>
	<path
		d="M25.6 2.1875H32L16 29.7875L0 2.1875H6.32H12.24L16 8.5875L19.68 2.1875H25.6Z"
		fill="#42B883"
	/>
	<path d="M0 2.1875L16 29.7875L32 2.1875H25.6L16 18.7475L6.32 2.1875H0Z" fill="#42B883" />
	<path d="M6.32 2.1875L16 18.8275L25.6 2.1875H19.68L16 8.5875L12.24 2.1875H6.32Z" fill="#35495E" />
</svg>
