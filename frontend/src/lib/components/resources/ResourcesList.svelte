<script lang="ts">
	...
	import ExploreAssetButton, { assetCanBeExplored } from '$lib/components/ExploreAssetButton.svelte'
	...
	async function loadResourceTypes(): Promise<void> {
		const rows = await ResourceService.listResourceType({ workspace: $operatingWorkspace! })
		setResourceTypeDisplayNames(rows)
		resourceTypes = rows.map((x) => {
			return {
				canWrite: $operatingWorkspace! == x.workspace_id,
				...x
			}
		})
		loading.types = false
	...
	async function deleteResource(path: string, account?: number): Promise<void> {
		...
		// page would show.
		sendUserToast(await ResourceService.deleteResource({ workspace: $operatingWorkspace!, path }))
		reload()
	}
	...
	async function handleDeleteResourceType(name: string) {
		try {
			await ResourceService.deleteResourceType({ workspace: $operatingWorkspace!, path: name })
			loadResourceTypes()
		} catch (err) {
	...
	onMount(() => {
		// Redirect targets of this route's own connect flows, never a host's.
		if (hosted) return
	...
	async function openResourceFromHash(path: string) {
		...
		if (resourceType === undefined) {
			...
			try {
				resourceType = (
					await ResourceService.getResource({ workspace: $operatingWorkspace!, path })
				).resource_type
			} catch (err) {
		...
		<div class="flex flex-col gap-6">
			<label for="inp">
				...
				<div class="flex flex-row items-center gap-x-4">
					...
					{#if operatingUser.current?.is_admin || operatingUser.current?.is_super_admin}
						<Toggle
...
{#if operatingUser.current?.operator && $operatingWorkspace && !$userWorkspaces.find((_) => _.id === $operatingWorkspace)?.operator_settings?.resources}
							...
							<tbody class="divide-y bg-surface">
								{#if filteredItems}
									{#each filteredItems as { path, description, resource_type, extra_perms, canWrite, is_oauth, is_linked, account, refresh_error, is_expired, marked, is_refreshed, labels, inherited_labels, ws_specific, draft_only, is_draft }}
										{@const hasDraft =
											getLocalDraftHint($operatingWorkspace, 'resource', path) ?? is_draft}
										<Row>
											<Cell first>
											...
											<Cell last actions>
												<div class="flex justify-end items-center gap-2">
													{#if path && assetCanBeExplored({ kind: 'resource', path }, { resource_type }) && !operatingUser.current?.operator}
														<ExploreAssetButton
																...
																? [
																		{
																			...
																			action: () => {
																				...
																				handledHash = `#/resource/${path}`
																				// A host's item tab opens an agent on JSON already.
																				if (hosted) hosted.openItem(path)
																				else resourceEditor?.initEdit?.(path, { json: true })
																			}
																		}
																	]
																: []),
