<script lang="ts">
	...
	function computeShouldDisplayDefaultValueInput(columnMetadata, value) {
		if (columnMetadata?.isnullable === 'NO' && !columnMetadata?.defaultvalue && value?.insert) {
			if ([ColumnIdentity.Always, ColumnIdentity.ByDefault].includes(columnMetadata?.isidentity)) {
				return false
			}
	...
	{#if value}
		<div class="flex flex-col w-96 p-2 gap-4">
			<Section label="Column settings">
				...
				{#if warning}
					<Alert type={warning.type} size="xs" title={warning.title} class="mt-2">
						{warning.message}
					</Alert>
				{/if}

				{#if shouldDisplayDefaultValueInput}
					<Label label="Default input">
						<svelte:fragment slot="header">
							<Tooltip>
								By default, all columns are used to generate the submit form. If you want to exclude
								a column from the submit form, you can set this property to true. If the column is
