 ...
 * resource brain and only then writes the flow-local inputs (`user_message`, `state`,
 * `user_attachments`, `enabled_tools`, `memory_id`, `previous_messages`) back from the step's own
 * args. `tool_inputs` stays untouched — the worker overlays it onto the tools in both branches, so an
 * inlined step keeps the host flow's tool bindings. The worker never reads a history input from the
 * resource, so one a draft happens to carry is left out here too.
 */
