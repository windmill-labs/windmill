	...
	properties: {
		...
		messages_context_length: {
			...
			'x-no-s3-storage-workspace-warning':
				'When no S3 storage is configured in your workspace settings, memory will be stored in database, which implies a limit of 500KB per memory entry. If you need to store more messages, you should use S3 storage in your workspace settings.',
			showExpr: "fields.output_type === 'text'"
		},
		output_schema: {
			type: 'object',
