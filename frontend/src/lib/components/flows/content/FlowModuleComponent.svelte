<script lang="ts">
	...
	const dapServerUrl = $derived(
		rawScriptLang ? (DAP_SERVER_URLS[rawScriptLang as keyof typeof DAP_SERVER_URLS] ?? DAP_SERVER_URLS.python3) : DAP_SERVER_URLS.python3
	)
	...
	function getDebugErrorMessage(error: unknown): string {
		...
		if (message.includes('Token verification failed') || message.includes('Debug token required')) {
			if (message.includes('expired')) return 'Debug session expired. Please try again.'
			if (message.includes('Invalid JWT signature')) return 'Debug authorization failed. The signing key may be misconfigured.'
			if (message.includes('Code hash mismatch')) return 'Code was modified after signing. Please try again.'
			if (message.includes('Public key not available')) return 'Debug server cannot verify tokens. Please check WINDMILL_BASE_URL configuration.'
			if (message.includes('Debug token required')) return 'Debug authorization required. The debug session must be signed by the backend.'
	...
	async function startDebugging(): Promise<void> {
		...
		try {
			...
			let signedPayload
			try {
				signedPayload = await signDebugRequest(code ?? '', rawScriptLang ?? 'python3')
