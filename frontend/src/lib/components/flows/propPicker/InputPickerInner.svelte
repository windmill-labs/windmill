...
{#snippet metaData(key: string)}
	{#if inputTransform?.[key]}
		<span
			class={twMerge(
				'inline-flex items-center h-4 border dark:bg-blue-200  px-1 rounded-[0.275rem] rounded-l-none border-l-0 gap-0.5',
				'text-2xs'
			)}
			title={inputTransform[key].type === 'javascript' ? inputTransform[key].expr : 'Static'}
		>
			{#if inputTransform[key].type === 'javascript'}
				<SquareFunction size={14} class="text-blue-500 -my-1 dark:text-blue-900" />
			...
			{#if testSteps?.manuallySetArgs[id]?.[key]}
				<button
					...
					class="-my-1 ml-0.5 hover:text-primary text-gray-300"
				>
					<RefreshCw size={12} />
				</button>
			{/if}
