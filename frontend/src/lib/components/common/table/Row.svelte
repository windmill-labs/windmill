<script lang="ts">
	...
	interface Props {
		...
		disabled?: boolean
		/** Ghosted look for items that exist only as a local draft (not yet in the workspace). */
		faded?: boolean
		canFavorite?: boolean
...
>
	...
	{#if href}
		<a
			...
			class={twMerge(
				'min-w-0 grow hover:underline decoration-gray-400 inline-flex items-center gap-4',
				faded ? 'opacity-60' : ''
			)}
		>
			{@render rowContent()}
	...
	<div
		data-row-actions
		class={twMerge('flex gap-1 items-center justify-end pr-2', faded ? 'opacity-60' : '')}
	>
		{@render actions?.()}
	</div>
</div>
