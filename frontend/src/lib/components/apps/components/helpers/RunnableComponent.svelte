	...
	>
		...
		{:else if getError(result) && $mode === 'preview' && !errorHandledByComponent}
			<div
				title="Error"
				class={classNames(
					'text-red-500 px-1 text-2xs py-0.5 font-bold w-fit absolute border border-red-500 -bottom-2  shadow-xs left-1/2 transform -translate-x-1/2 z-50 cursor-pointer'
				)}
			>
