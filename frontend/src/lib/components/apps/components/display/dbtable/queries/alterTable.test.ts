...
import { makeAlterTableQuery } from "./alterTable";

function normalize(sql: string) {
  return sql.replace(/\s+/g, " ").trim();
}

describe("makeAlterTableQuery", () => {
  it("adds a column", () => {
    const queries = makeAlterTableQuery(
      {
        name: "users",
        operations: [
          {
  ...
  it("drops a column", () => {
    ...
    expect(normalize(queries[0])).toBe(
      normalize("ALTER TABLE users DROP COLUMN age;"),
    );
  });
  ...
  it("sets and drops default values", () => {
    ...
    expect(normalize(queries[1])).toEqual(
