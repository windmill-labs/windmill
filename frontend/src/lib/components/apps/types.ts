...
export type App = {
	 ...
	 * from `value->>'path'`) so a friendly name shows up instead of the
	 * autogenerated `u/{user}/draft_{uuid}` storage key for renames-in-progress
	 * and brand-new drafts. Dropped from the JSON once the typed path matches the
	 * baseline again, and deploy clears the whole draft. The deployed/original
	 ...
	 * normally lives in the `app` table column, set only on deploy). Mirrors the
	 * `path` field above: draft-only metadata; the deployed summary column is
