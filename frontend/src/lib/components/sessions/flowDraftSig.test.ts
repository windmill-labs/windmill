...
describe('flowDraftSig', () => {
	...
	it('changes when only path changes (a rename triggers a save)', () => {
		const base = { value: { modules: [] }, summary: 's', path: 'u/admin/draft_abc' }
		expect(flowDraftSig(base)).not.toBe(flowDraftSig({ ...base, path: 'u/admin/friendly' }))
	})
})
