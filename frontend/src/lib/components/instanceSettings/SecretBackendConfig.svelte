<script lang="ts">
	...
	import { Database, Lock, Server, ArrowLeft, ArrowRight, Cloud, KeyRound } from 'lucide-svelte'
	...
	let selectedType: 'Database' | 'HashiCorpVault' | 'AzureKeyVault' | 'AwsKms' = $derived(
		$values['secret_backend']?.type ?? 'Database'
	)
	...
	let testingAwsKmsConnection = $state(false)
	let migratingToAwsKms = $state(false)
	let migratingFromAwsKms = $state(false)
	let migrateToAwsKmsModalOpen = $state(false)
	let migrateFromAwsKmsModalOpen = $state(false)

	let vaultDisabled = $derived(!$enterpriseLicense)

	function setBackendType(type: string | undefined) {
		if (!type) return
		if (type === 'HashiCorpVault' && vaultDisabled) return
		if (type === 'AzureKeyVault' && vaultDisabled) return
		if (type === 'AwsKms' && vaultDisabled) return
		...
		if (type === 'Database') {
			...
			$values['secret_backend'] = {
				type: 'AwsKms',
				key_id: $values['secret_backend']?.key_id ?? '',
	...
	async function testAwsKmsConnection() {
		if (!$values['secret_backend'] || $values['secret_backend'].type !== 'AwsKms') return
		testingAwsKmsConnection = true
		try {
			await SettingService.testAwsKmsBackend({ requestBody: getAwsKmsSettings() })
			sendUserToast('Successfully connected to AWS KMS')
		} catch (error: any) {
			sendUserToast('Failed to connect to AWS KMS: ' + error.message, true)
		} finally {
			testingAwsKmsConnection = false
		}
	}

	async function migrateSecretsToAwsKms() {
		if (!$values['secret_backend'] || $values['secret_backend'].type !== 'AwsKms') return
		migratingToAwsKms = true
		try {
			const report = await SettingService.migrateSecretsToAwsKms({ requestBody: getAwsKmsSettings() })
			if (report.failed_count > 0) {
				sendUserToast(`Migration completed with errors: ${report.migrated_count}/${report.total_secrets} secrets migrated, ${report.failed_count} failed`, true)
			} else {
				sendUserToast(`Successfully migrated ${report.migrated_count}/${report.total_secrets} secrets to AWS KMS`)
			}
		} catch (error: any) {
			sendUserToast('Failed to migrate secrets to AWS KMS: ' + error.message, true)
		} finally {
			migratingToAwsKms = false
			migrateToAwsKmsModalOpen = false
		}
	}

	async function migrateSecretsFromAwsKms() {
		if (!$values['secret_backend'] || $values['secret_backend'].type !== 'AwsKms') return
		migratingFromAwsKms = true
		try {
			const report = await SettingService.migrateSecretsFromAwsKms({ requestBody: getAwsKmsSettings() })
			if (report.failed_count > 0) {
				sendUserToast(`Migration completed with errors: ${report.migrated_count}/${report.total_secrets} secrets migrated, ${report.failed_count} failed`, true)
			} else {
				sendUserToast(`Successfully migrated ${report.migrated_count}/${report.total_secrets} secrets to database`)
			...
			sendUserToast('Failed to migrate secrets from AWS KMS: ' + error.message, true)
		} finally {
			migratingFromAwsKms = false
			migrateFromAwsKmsModalOpen = false
		}
	}

	function isAwsKmsConfigValid(): boolean {
		if (!$values['secret_backend'] || $values['secret_backend'].type !== 'AwsKms') return false
		return (
			$values['secret_backend'].key_id?.trim() !== '' &&
			$values['secret_backend'].region?.trim() !== ''
		)
	}
	...
	<div class="flex flex-col gap-2 mt-1">
		<ToggleButtonGroup selected={selectedType} onSelected={(v) => setBackendType(v)}>
			{#snippet children({ item: toggleButton })}
				...
				<ToggleButton
					...
					label="AWS KMS"
					tooltip={vaultDisabled
						? 'AWS KMS integration requires Enterprise Edition'
						: 'Encrypt secrets with AWS KMS (stored in database)'}
					item={toggleButton}
					disabled={vaultDisabled}
				/>
			{/snippet}
		...
		{#if vaultDisabled}
			<div class="flex items-center gap-1">
				<EEOnly>HashiCorp Vault, Azure Key Vault, and AWS KMS integrations require Enterprise Edition</EEOnly>
			</div>
		{/if}
	</div>
  ...
  ttl="1h"</pre
					...
					<div class="flex gap-4">
						<div class="flex-1 p-3 border rounded-lg">
							<div class="flex items-center gap-2 mb-2">
								<Database size={16} /><ArrowRight size={16} /><Lock size={16} />
							</div>
							<p class="text-xs font-medium mb-2">Database → Vault</p>
							<p class="text-2xs text-secondary mb-3">Decrypt secrets from database and store in Vault</p>
							<Button unifiedSize="sm" variant="default" onclick={() => (migrateToVaultModalOpen = true)} disabled={disabled || !isVaultConfigValid() || migratingToVault} startIcon={{ icon: ArrowRight }}>
								Migrate to Vault
							</Button>
						</div>
						<div class="flex-1 p-3 border rounded-lg">
							<div class="flex items-center gap-2 mb-2">
								<Lock size={16} /><ArrowLeft size={16} /><Database size={16} />
							</div>
							<p class="text-xs font-medium mb-2">Vault → Database</p>
							<p class="text-2xs text-secondary mb-3">Read secrets from Vault and encrypt in database</p>
							<Button unifiedSize="sm" variant="default" onclick={() => (migrateToDatabaseModalOpen = true)} disabled={disabled || !isVaultConfigValid() || migratingToDatabase} startIcon={{ icon: ArrowLeft }}>
								Migrate to Database
							</Button>
						</div>
					</div>
	...
	{:else if selectedType === 'AzureKeyVault'}
		...
		<div class="space-y-4 p-4 border rounded-lg">
			...
			<div class="flex flex-col gap-4 pt-4 border-t">
				<div class="flex gap-2">
					<Button unifiedSize="md" variant="accent" onclick={testAzureKvConnection} disabled={disabled || !isAzureKvConfigValid() || testingAzureKvConnection} loading={testingAzureKvConnection} startIcon={{ icon: Server }}>
						Test Connection
					</Button>
				</div>
				<div class="flex flex-col gap-4 pt-4 border-t">
					<span class="block text-xs font-semibold text-emphasis">Secret Migration</span>
					<span class="text-2xs text-secondary">Migrate secrets between the database and Azure Key Vault. Original values are NOT deleted to allow for rollback.</span>
					<div class="flex gap-4">
						<div class="flex-1 p-3 border rounded-lg">
							...
							<p class="text-2xs text-secondary mb-3">Decrypt secrets from database and store in Azure Key Vault</p>
							<Button unifiedSize="sm" variant="default" onclick={() => (migrateToAzureKvModalOpen = true)} disabled={disabled || !isAzureKvConfigValid() || migratingToAzureKv} startIcon={{ icon: ArrowRight }}>Migrate to Azure Key Vault</Button>
						</div>
						<div class="flex-1 p-3 border rounded-lg">
							...
							<p class="text-2xs text-secondary mb-3">Read secrets from Azure Key Vault and encrypt in database</p>
	...
	{:else if selectedType === 'AwsKms'}
		<!-- AWS KMS Configuration -->
		<div class="space-y-4 p-4 border rounded-lg">
			<div class="flex items-center gap-2 mb-4">
				<KeyRound class="text-primary" size={20} />
				<div>
					<p class="text-sm font-medium text-emphasis">AWS KMS Configuration</p>
					<p class="text-xs text-secondary">
						Encrypt secrets using AWS Key Management Service. Secrets remain stored in the database
						but are encrypted/decrypted by KMS instead of Windmill's built-in encryption.
					</p>
				</div>
			</div>
			...
			<div class="grid grid-cols-1 gap-4">
				<div class="flex flex-col gap-1">
					<label for="aws_kms_key_id" class="block text-xs font-semibold text-emphasis">KMS Key ID</label>
					<span class="text-2xs text-secondary">Key ID, Key ARN, Alias name (e.g. alias/my-key), or Alias ARN</span>
					<TextInput inputProps={{ type: 'text', id: 'aws_kms_key_id', placeholder: 'alias/windmill-secrets', disabled }} bind:value={$values['secret_backend'].key_id} />
				</div>

				<div class="flex flex-col gap-1">
					<label for="aws_kms_region" class="block text-xs font-semibold text-emphasis">Region</label>
					<TextInput inputProps={{ type: 'text', id: 'aws_kms_region', placeholder: 'us-east-1', disabled }} bind:value={$values['secret_backend'].region} />
				</div>

				<div class="flex flex-col gap-1">
					<label for="aws_kms_access_key_id" class="block text-xs font-semibold text-emphasis">Access Key ID (optional)</label>
					...
					<TextInput inputProps={{ type: 'text', id: 'aws_kms_access_key_id', placeholder: 'AKIA...', disabled }} bind:value={$values['secret_backend'].access_key_id} />
				</div>

				<div class="flex flex-col gap-1 p-3 bg-surface-secondary rounded-lg">
					<label for="aws_kms_secret_access_key" class="block text-xs font-semibold text-emphasis">Secret Access Key (optional)</label>
					<Password bind:password={$values['secret_backend'].secret_access_key} small {disabled} />
				</div>

				<div class="flex flex-col gap-1">
					<label for="aws_kms_endpoint_url" class="block text-xs font-semibold text-emphasis">Endpoint URL (optional)</label>
					<span class="text-2xs text-secondary">Custom endpoint for LocalStack or other KMS-compatible services</span>
					<TextInput inputProps={{ type: 'text', id: 'aws_kms_endpoint_url', placeholder: 'http://localhost:4566', disabled }} bind:value={$values['secret_backend'].endpoint_url} />
				</div>
			</div>
			...
			<div class="flex flex-col gap-4 pt-4 border-t">
				<div class="flex gap-2">
					<Button
						unifiedSize="md"
						variant="accent"
						onclick={testAwsKmsConnection}
						disabled={disabled || !isAwsKmsConfigValid() || testingAwsKmsConnection}
						loading={testingAwsKmsConnection}
						startIcon={{ icon: Server }}
					>
						Test Connection
					</Button>
				...
				<div class="flex flex-col gap-4 pt-4 border-t">
					<span class="block text-xs font-semibold text-emphasis">Secret Migration</span>
					<span class="text-2xs text-secondary">
						Migrate secrets between database encryption (workspace keys) and AWS KMS encryption.
						Secrets stay in the database; only the encryption method changes.
					</span>
					<div class="flex gap-4">
						<div class="flex-1 p-3 border rounded-lg">
							<div class="flex items-center gap-2 mb-2">
								<Database size={16} /><ArrowRight size={16} /><KeyRound size={16} />
							</div>
							<p class="text-xs font-medium mb-2">Database → AWS KMS</p>
							<p class="text-2xs text-secondary mb-3">Re-encrypt secrets from workspace keys to AWS KMS</p>
							<Button unifiedSize="sm" variant="default" onclick={() => (migrateToAwsKmsModalOpen = true)} disabled={disabled || !isAwsKmsConfigValid() || migratingToAwsKms} startIcon={{ icon: ArrowRight }}>
								Migrate to AWS KMS
							</Button>
						</div>
						<div class="flex-1 p-3 border rounded-lg">
							<div class="flex items-center gap-2 mb-2">
								<KeyRound size={16} /><ArrowLeft size={16} /><Database size={16} />
							</div>
							<p class="text-xs font-medium mb-2">AWS KMS → Database</p>
							<p class="text-2xs text-secondary mb-3">Re-encrypt secrets from AWS KMS to workspace keys</p>
							<Button unifiedSize="sm" variant="default" onclick={() => (migrateFromAwsKmsModalOpen = true)} disabled={disabled || !isAwsKmsConfigValid() || migratingFromAwsKms} startIcon={{ icon: ArrowLeft }}>
								Migrate to Database
							</Button>
						</div>
					</div>
...
<!-- Migrate to AWS KMS Modal -->
<ConfirmationModal
	title="Migrate Secrets to AWS KMS"
	confirmationText="Migrate"
	open={migrateToAwsKmsModalOpen}
	loading={migratingToAwsKms}
	type="reload"
	onCanceled={() => { migrateToAwsKmsModalOpen = false }}
	onConfirmed={migrateSecretsToAwsKms}
>
	{#snippet children()}
		<div class="flex flex-col gap-2">
			<p>This will re-encrypt all existing secrets using AWS KMS. The process will:</p>
			<ol class="list-decimal list-inside text-sm space-y-1">
				<li>Read all encrypted secrets from the database</li>
				<li>Decrypt them using the workspace encryption keys</li>
				<li>Re-encrypt them using AWS KMS</li>
				<li>Update the database values in-place</li>
			</ol>
			<p class="text-yellow-600 dark:text-yellow-400 text-sm mt-2">
				Note: Secrets remain in the database. The encryption method changes from workspace keys to AWS KMS.
			</p>
...
<!-- Migrate from AWS KMS to Database Modal -->
<ConfirmationModal
	title="Migrate Secrets to Database"
	confirmationText="Migrate"
	open={migrateFromAwsKmsModalOpen}
	loading={migratingFromAwsKms}
	type="reload"
	onCanceled={() => { migrateFromAwsKmsModalOpen = false }}
	onConfirmed={migrateSecretsFromAwsKms}
>
	{#snippet children()}
		<div class="flex flex-col gap-2">
			<p>This will re-encrypt all AWS KMS-encrypted secrets using workspace keys. The process will:</p>
			<ol class="list-decimal list-inside text-sm space-y-1">
				<li>Find all secrets encrypted with AWS KMS</li>
				<li>Decrypt them using AWS KMS</li>
				<li>Re-encrypt them using workspace encryption keys</li>
				<li>Update the database values in-place</li>
			</ol>
			<p>Are you sure you want to proceed?</p>
		</div>
...
<ConfirmationModal title="Migrate Secrets to Azure Key Vault" confirmationText="Migrate" open={migrateToAzureKvModalOpen} loading={migratingToAzureKv} type="reload" onCanceled={() => { migrateToAzureKvModalOpen = false }} onConfirmed={migrateSecretsToAzureKv}>
	{#snippet children()}
		<div class="flex flex-col gap-2">
			<p>This will migrate all existing secrets from the database to Azure Key Vault. The process will:</p>
			<ol class="list-decimal list-inside text-sm space-y-1">
				<li>Read all encrypted secrets from the database</li>
				<li>Decrypt them using the workspace encryption keys</li>
				<li>Store them in Azure Key Vault</li>
			</ol>
			<p class="text-yellow-600 dark:text-yellow-400 text-sm mt-2">Note: Database values are NOT deleted automatically.</p>
			<p>Are you sure you want to proceed?</p>
		</div>
	{/snippet}
</ConfirmationModal>
...
<ConfirmationModal title="Migrate Secrets to Database" confirmationText="Migrate" open={migrateFromAzureKvModalOpen} loading={migratingFromAzureKv} type="reload" onCanceled={() => { migrateFromAzureKvModalOpen = false }} onConfirmed={migrateSecretsFromAzureKv}>
	{#snippet children()}
		<div class="flex flex-col gap-2">
			<p>This will migrate all secrets from Azure Key Vault back to the database.</p>
			<p class="text-yellow-600 dark:text-yellow-400 text-sm mt-2">Note: Azure Key Vault values are NOT deleted automatically.</p>
			<p>Are you sure you want to proceed?</p>
		</div>
	{/snippet}
</ConfirmationModal>
...
<ConfirmationModal title="Migrate Secrets to Vault" confirmationText="Migrate" open={migrateToVaultModalOpen} loading={migratingToVault} type="reload" onCanceled={() => { migrateToVaultModalOpen = false }} onConfirmed={migrateSecretsToVault}>
	{#snippet children()}
		<div class="flex flex-col gap-2">
			<p>This will migrate all existing secrets from the database to HashiCorp Vault.</p>
			<p class="text-yellow-600 dark:text-yellow-400 text-sm mt-2">Note: Database values are NOT deleted automatically.</p>
			<p>Are you sure you want to proceed?</p>
		</div>
	{/snippet}
...
<ConfirmationModal title="Migrate Secrets to Database" confirmationText="Migrate" open={migrateToDatabaseModalOpen} loading={migratingToDatabase} type="reload" onCanceled={() => { migrateToDatabaseModalOpen = false }} onConfirmed={migrateSecretsToDatabase}>
	{#snippet children()}
		<div class="flex flex-col gap-2">
			<p>This will migrate all secrets from HashiCorp Vault back to the database.</p>
			<p class="text-yellow-600 dark:text-yellow-400 text-sm mt-2">Note: Vault values are NOT deleted automatically.</p>
			<p>Are you sure you want to proceed?</p>
		</div>
	{/snippet}
</ConfirmationModal>
