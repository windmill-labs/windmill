...
- Before writing your own code against an integration the hub covers, call get_hub_integration with its slug: it returns the resource type to take, the auth, pagination, enums, error codes and known gotchas in one call, which beats inferring them from script bodies. Only an integration a search marked \`documented\` holds that provider knowledge, and few do; for the rest the call adds the resource type and little the scripts do not already show, so read a script instead.${webSearchBullet}
