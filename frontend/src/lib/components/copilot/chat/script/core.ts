...
function buildChatSystemPrompt(currentModel: AIProviderModel) {
	...
	- Use \`create_schedule\` or \`create_trigger\` when the user asks to create a schedule or trigger for the current script. These tools automatically infer the current script path; never provide \`script_path\` or \`is_flow\`. If the current script has no saved path, the tool will return a clear error that you should explain to the user.
