...
export type ToolDisplayMessage = {
	 ...
	 * reads its panes from this job, so the user sees the full args, result and logs while
	 * the model keeps the capped envelope the tool returned. `runId` and `step` are the
	 * address the call was made with, kept so the card can name what was inspected the way
	 * the tool's own row did: a step job names neither the step nor the run it belongs to. */
