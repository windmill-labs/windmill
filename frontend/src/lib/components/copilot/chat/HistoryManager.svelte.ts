	...
	async saveChat(
		...
		if (displayMessages.length > 0) {
			...
			const updatedChat = {
				...
				// the previously saved field. Snapshot the fallback: savedChats is
				// $state, so the stored value is a proxy that structuredClone (used by
				// IndexedDB put) cannot clone.
