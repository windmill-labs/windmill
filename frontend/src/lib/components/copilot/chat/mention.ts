...
/** Matches a mention token: `@[name with spaces]` first, then a bare `@name`. */
export const MENTION_RE = /@\[[^\]\r\n]+\]|@[\w/.\-\[\]]+/g

/** The title of a mention token (`@name` or `@[name]`), brackets stripped. */
export function mentionTitle(token: string): string {
	return token.startsWith('@[') && token.endsWith(']') ? token.slice(2, -1) : token.slice(1)
}

/** Format a name as a mention token — bracket it when it contains whitespace. */
