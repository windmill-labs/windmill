...
- \`index.tsx\` is also the **mount** entrypoint: nothing is auto-rendered, so it must mount a top-level \`App\` into \`#root\` itself. Keep the UI in \`App.tsx\` and keep \`index.tsx\` as the mount shim:
  \`\`\`tsx
...
- **Never replace \`index.tsx\` with a bare component.** A component that is defined but never mounted renders a blank screen with **no error** — the JSX never executes, so nothing throws. If an app renders blank, check that \`index.tsx\` still calls \`createRoot(document.getElementById('root')!).render(<App />)\`.
