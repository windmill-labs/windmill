...
On a workspace-leaf pick, the chat needs the full body of the script or
flow — fetched here via `workspaceRunnablesSearch` and passed to
`onSelectWorkspaceItem` as the existing `WorkspaceScriptElement` /
`WorkspaceFlowElement` shapes.
-->
<script lang="ts">
	...
	import { MAX_RUNNABLE_CONTENT_LENGTH, workspaceRunnablesSearch } from './shared'
	import {
		ContextIconMap,
		type ContextElement,
	...
	async function handlePick(leaf: DrillLeaf<ChatLeafData>) {
		const d = leaf.data
		if ('kind' in d) {
			// Workspace item — fetch content and emit as a workspace_* element.
			const workspace = $workspaceStore
			if (!workspace || !onSelectWorkspaceItem) return
			try {
				if (d.kind === 'script') {
					const script = await workspaceRunnablesSearch.getScript(d.path, workspace)
					...
					const flow = await workspaceRunnablesSearch.getFlow(d.path, workspace)
