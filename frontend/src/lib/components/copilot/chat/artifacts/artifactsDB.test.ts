...
describe('artifactsDB', () => {
	...
	it('keeps only the most recent versions of an artifact', async () => {
		...
		for (let v = 1; v <= total; v++) await db.putArtifactVersion(version(v))
	...
	it('keeps fewer versions of a large artifact, but never fewer than the minimum', async () => {
		...
		for (let v = 1; v <= 8; v++) await db.putArtifactVersion({ ...version(v), content: big })
		...
		await db.putArtifactVersion({ ...version(9), content: huge })
	...
	it('deleting an artifact, or a whole session, drops the versions with it', async () => {
		...
		await db.putArtifactVersion(version(1))
		await db.putArtifactVersion(version(1, 'a2'))
