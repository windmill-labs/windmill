<script lang="ts">
	...
	async function load() {
		...
		const loaded = await store.listVersions(id)
		if (artifact.id === id) versions = loaded
	}
		...
		<div class="flex w-80 flex-col text-xs">
			...
			<div class="max-h-[min(20rem,60vh)] overflow-y-auto py-1" role="listbox" tabindex="-1">
				{#each versions as v (v.version)}
					...
					>
						<!-- The note gets the full width and up to two lines: a change note is the
						     thing you scan for, and one truncated line is not enough to tell two
						     edits apart. Version and time drop to a second, quieter line. -->
