<script lang="ts">
	...
	let fetchedOpenApiSpecFromUrl = $state(false)
...
<Section label="Http routes generator">
	<Subsection>
		...
		<div class="flex flex-col gap-2 mt-2">
			...
			{/if}
			{#if selected === 'OpenApi' || (selected === 'OpenApiFile' && !emptyStringTrimmed(openApiFile)) || (selected === 'OpenApiURL' && (!emptyStringTrimmed(openApiUrl) || fetchedOpenApiSpecFromUrl))}
				<div class="h-96">
			...
			<Button
				...
				on:click={async () => {
					...
					httpTriggers = await generateHttpTriggerFromOpenApi(code)
				}}
				color="light"
