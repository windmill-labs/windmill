						...
						{#if $values[setting.key]}
							<div class="flex gap-8">
								<Toggle
									...
									id="tracing_enabled"
									bind:checked={
										() => $values[setting.key].tracing_enabled ?? false,
										(v) => ($values[setting.key].tracing_enabled = v)
									}
								...
								<Toggle
									...
									id="logs_enabled"
									bind:checked={
										() => $values[setting.key].logs_enabled ?? false,
										(v) => ($values[setting.key].logs_enabled = v)
									}
									options={{ right: 'Logs' }}
								/>
								<Toggle
