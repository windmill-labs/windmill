...
/**
 * Container-level network configuration that the debugged process, and more importantly the
 * `windmill prepare-deps` subprocess the Python DAP server shells out to, cannot work without:
 * behind a corporate proxy or a private package index, `uv venv` / `uv pip install` reach nothing
 * unless these are handed down. They are read from the service's own environment because there is
 * no other channel a deployment can configure them through.
 */
...
function spawnProcess(options: SpawnOptions): Subprocess {
	...
	// Only include essential env vars + the network-config allowlist + caller-provided ones.
