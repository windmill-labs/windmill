...
import {
  applyDatatableSql,
  type BenchmarkDatatableSeed,
} from "./datatableSqlEngine";

export type {
  BenchmarkDatatableSeed,
  BenchmarkDatatableTableSeed,
} from "./datatableSqlEngine";
...
const benchmarkWorkspaceRunnables = new Map<
  string,
  BenchmarkWorkspaceRunnables
>();
...
const benchmarkJobs = new Map<
  string,
  { workspace: string; job: BenchmarkCompletedJob }
>();
...
export function createBenchmarkFolder(
  _workspace: string,
  name: string,
): string {
  return name;
...
export function registerBenchmarkWorkspaceRunnables(
  ...
  benchmarkWorkspaceRunnables.set(workspace, {
    ...runnables,
    datatables: runnables.datatables
      ? structuredClone(runnables.datatables)
      : undefined,
  });
...
export function getBenchmarkScriptByPath(
  workspace: string,
  path: string,
): Script | null {
...
export function getBenchmarkScriptByHash(
  workspace: string,
  hash: string,
): Script | null {
  const script = benchmarkWorkspaceRunnables
...
export function getBenchmarkFlowByPath(
  workspace: string,
  path: string,
): Flow | null {
...
export function getBenchmarkAppByPath(
  workspace: string,
  path: string,
): AppWithLastVersion | null {
...
export function getBenchmarkDraftForUser(input: {
  ...
  const entry = benchmarkDrafts.get(
    benchmarkDraftKey(input.workspace, input.kind, input.path),
  );
  if (!entry) {
...
function benchmarkDeployedExists(
  ...
  if (kind === "app" || kind === "raw_app")
    return Boolean(getBenchmarkAppByPath(workspace, path));
 ...
 */
export function listBenchmarkDatatables(
  workspace: string,
): DataTableTables[] | null {
  const runnables = benchmarkWorkspaceRunnables.get(workspace);
  if (!runnables) {
    return null;
  }
  return (runnables.datatables ?? []).map((datatable) => ({
    ...
    schemas: Object.fromEntries(
      Object.entries(datatable.schemas).map(([schema, tables]) => [
        schema,
        Object.keys(tables),
      ]),
    ),
...
export function previewBenchmarkSchedule(input: {
  ...
  if (
    ...
    schedule.trim().split(/\s+/).length !== 6
  ) {
    throw new Error(
...
function assertBenchmarkWorkspacePath(label: string, value: unknown): void {
  if (
    typeof value !== "string" ||
    (!value.startsWith("f/") && !value.startsWith("u/"))
  ) {
    throw new Error(
      `${label} path must start with f/ or u/, got ${JSON.stringify(value)}`,
    );
  }
}
...
const BENCHMARK_INTEGRATION_META_PATH =
  /^\/api\/integrations\/hub\/([^/]+)\/meta$/;
...
function searchBenchmarkHubScripts(
  text: string,
  app: string | null,
  kind: string | null,
) {
  ...
  return BENCHMARK_HUB_SCRIPTS.filter(
    ...
    .map((script) => {
      const words = new Set(
        `${script.app} ${script.summary} ${script.terms}`
          .toLowerCase()
          .split(/[^a-z0-9]+/),
      );
...
function benchmarkDerivedFacts(app: string) {
  ...
  for (const script of scripts) {
    ...
    for (const match of script.content.matchAll(
      /https?:\/\/([a-zA-Z0-9._-]+)/g,
    )) {
...
function getBenchmarkHubScript(path: string) {
  const versionId = Number(
    path.replace(/^\/api\/scripts\/hub\/get_full\/hub\//, "").split("/")[0],
  );
  return BENCHMARK_HUB_SCRIPTS.find(
    (script) => script.version_id === versionId,
  );
}
 ...
 * chat's hub tools. */
export function handleBenchmarkApiFetch(
  url: string,
  init?: RequestInit,
): Response {
  const path = url.split("?")[0];
  ...
  if (integrationMeta) {
    ...
    const real = REAL_HUB_INTEGRATIONS.find(
      (integration) => integration.app === app,
    );
    if (real) {
      return Response.json({
        app,
  ...
  if (path === "/api/integrations/hub/list") {
    const apps = [
      ...new Set(BENCHMARK_HUB_SCRIPTS.map((script) => script.app)),
    ].sort();
  ...
  return Response.json(
    { error: `no benchmark handler for ${path}` },
    { status: 404 },
  );
}
