...
- \`sync push\` - Push any local changes and apply them remotely.
  ...
  - \`--delete-untracked-secrets\` - Delete remote variables and resources this repository's history has never tracked. They are kept by default: unlike a script (which push archives) they are deleted irrecoverably, and one that was never in the repo was provisioned outside it rather than deleted from it.
  - \`--skip-branch-validation\` - Skip git branch validation and prompts
  - \`--json-output\` - Output results in JSON format
