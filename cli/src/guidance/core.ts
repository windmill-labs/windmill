...
While a \`wm-fork/<base>/<id>\` branch is checked out, every wmill command automatically targets the fork workspace (resolved from the branch name), reusing the base branch's auth and remote from wmill.yaml — no \`--workspace\` flag or profile switch needed. Pass \`--workspace\` to target a different workspace explicitly.
