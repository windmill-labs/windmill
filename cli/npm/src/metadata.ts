...
import { GlobalDeps, exts } from "./script.js";
import {
  FSFSElement,
...
import { FlowFile, replaceInlineScripts } from "./flow.js";
...
export async function generateFlowLockInternal(
  ...
  const flowValue = yamlParse(
    await dntShim.Deno.readTextFile(folder! + SEP + "flow.yaml")
  ) as FlowFile;
  ...
  if (!justUpdateMetadataLock) {
    ...
    inlineScripts
      ...
      .forEach((s) => {
        dntShim.Deno.writeTextFileSync(
          dntShim.Deno.cwd() + SEP + folder + SEP + s.path,
          s.content
        );
      });
  }
...
export async function parseMetadataFile(
  ...
  try {
    ...
    try {
      ...
      await dntShim.Deno.stat(metadataFilePath);
      const payload: any = yamlParse(await dntShim.Deno.readTextFile(metadataFilePath));
      replaceLock(payload);
      ...
      if (generateMetadataIfMissing) {
        ...
        try {
          ...
          scriptInitialMetadata = yamlParse(
            await dntShim.Deno.readTextFile(metadataFilePath)
          ) as ScriptMetadata;
