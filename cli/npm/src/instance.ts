...
import { sleep, deepEqual } from "./utils.js";
...
async function appendInstance(instance: Instance) {
  ...
  const file = await dntShim.Deno.open((await getRootStore()) + "instances.ndjson", {
    append: true,
    write: true,
    read: true,
    create: true,
    ...
    if (localWorkspacesToDelete.length > 0) {
      ...
      if (confirmDelete) {
        for (const workspace of localWorkspacesToDelete) {
          ...
          await dntShim.Deno.remove(path.join(rootDir, workspace.name), {
            recursive: true,
          });
        }
      }
...
async function instancePush(opts: GlobalOptions & InstanceSyncOptions) {
  ...
  if (opts.includeWorkspaces) {
    ...
    for (const localWorkspace of localWorkspaces) {
      ...
      sleep(1000);
      try {
        await dntShim.Deno.chdir(path.join(rootDir, localWorkspace.name));
      } catch (_) {
        throw new Error(
      ...
      try {
        const workspaceSettings = yamlParse(
          await dntShim.Deno.readTextFile("settings.yaml")
