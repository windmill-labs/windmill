...
export async function handleFile(path, workspace, alreadySynced, message, opts, globalDeps, codebases) {
    if (!path.includes(".inline_script.") &&
        ...
        const typed = (await parseMetadataFile(remotePath, opts
            ? {
                ...opts,
                path,
        ...
        if (codebase) {
            ...
            if (typedBefore != typed.schema) {
                ...
                showDiff(yamlStringify(typedBefore, yamlOptions), yamlStringify(typed.schema, yamlOptions));
                await dntShim.Deno.writeTextFile(remotePath + ".script.yaml", yamlStringify(typed, yamlOptions));
            }
        }
