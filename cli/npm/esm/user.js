...
export async function pushGroup(workspace, path, group, localGroup) {
...
export async function pullInstanceUsers(preview = false) {
    ...
    if (preview) {
        ...
        await dntShim.Deno.writeTextFile("instance_users.yaml", yamlStringify(remoteUsers));
...
export async function pullInstanceGroups(preview = false) {
    ...
    if (preview) {
        ...
        await dntShim.Deno.writeTextFile("instance_groups.yaml", yamlStringify(remoteGroups));
...
const command = new Command()
    ...
    .option("--company <company:string>", "Specify to set the company of the new user.")
    ...
    .option("--email <email:string>", "Specify credentials to use for authentication. This will not be stored. It will only be used to exchange for a token with the API server, which will not be stored either.", {
    depends: ["password"],
})
