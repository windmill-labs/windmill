    ...
    return Object.keys(subset).every((key) => {
        ...
        if (!eq && showDiffs) {
            ...
            if (!supers) {
                ...
                showDiff(yamlStringify(sub, yamlOptions), yamlStringify(supers, yamlOptions));
            }
        }
        return eq;
    });
}
export function showDiff(local, remote) {
    let finalString = "";
...
export function showConflict(path, local, remote) {
...
export function parseFromFile(p) {
    if (p.endsWith(".json")) {
        ...
        return yamlParse(dntShim.Deno.readTextFileSync(p));
    }
    else {
