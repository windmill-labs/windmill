...
import { colors, Command, Confirm, ensureDir, minimatch, path, ScriptService, FolderService, ResourceService, VariableService, AppService, FlowService, log, yamlStringify, yamlParse, ScheduleService, SEP, gitignore_parser, UserService, GroupService, } from "./deps.js";
...
async function addCodebaseDigestIfRelevant(path, content, codebases) {
    ...
    let isTs = true;
    try {
        await dntShim.Deno.stat(path.replace(".script.yaml", ".ts"));
    }
    catch {
        isTs = false;
    }
...
export async function ignoreF(wmillconf) {
    ...
    let ign = undefined;
    try {
        const ignoreContent = await dntShim.Deno.readTextFile(".wmillignore");
...
`));
        ...
        if (changedApps.length > 0) {
            log.info(`Apps ${changedApps.join(", ")} scripts were changed but ignoring for now`);
        }
