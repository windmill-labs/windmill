...
async function getEntity(path, mtime, stats, maxbuffer, response) {
    ...
    if (stats.size < maxbuffer) {
        const buffer = await (0, deps_js_1.readAll)(file);
        file.close();
        body = entity = buffer;
    }
    else {
...
async function sendRange(response, body, range, size) {
    const ranges = (0, range_js_1.parseRange)(range, size);
    if (ranges.length === 0) {
        throw (0, deps_js_1.createHttpError)(deps_js_1.Status.RequestedRangeNotSatisfiable);
    }
    response.status = deps_js_1.Status.PartialContent;
    if (ranges.length === 1) {
        ...
        if (body instanceof Uint8Array) {
            ...
            response.body = new deps_js_1.LimitedReader(body, byteRange.end - byteRange.start + 1);
        }
    }
    else {
        (0, util_js_1.assert)(response.type);
        if (!boundary) {
            boundary = await (0, util_js_1.getBoundary)();
        }
        response.headers.set("content-type", `multipart/byteranges; boundary=${boundary}`);
        const multipartBody = new range_js_1.MultiPartStream(body, response.type, ranges, size, boundary);
...
{ request, response }, path, options = { root: "" }) {
    ...
    path = (0, util_js_1.decodeComponent)(path.substr((0, deps_js_1.parse)(path).root.length));
    ...
    path = (0, util_js_1.resolvePath)(root, path);
    let encodingExt = "";
    if (brotli &&
    ...
    try {
        ...
        if (err instanceof dntShim.Deno.errors.NotFound) {
            throw (0, deps_js_1.createHttpError)(404, err.message);
        }
        // TODO(@kitsonk) remove when https://github.com/denoland/node_deno_shims/issues/87 resolved
        if (err instanceof Error && err.message.startsWith("ENOENT:")) {
            throw (0, deps_js_1.createHttpError)(404, err.message);
        }
        throw (0, deps_js_1.createHttpError)(500, err instanceof Error ? err.message : "[non-error thrown]");
    }
    let mtime = null;
    ...
    if (!response.type) {
        response.type = encodingExt !== ""
            ? (0, deps_js_1.extname)((0, deps_js_1.basename)(path, encodingExt))
            : contentTypes[(0, deps_js_1.extname)(path)] ?? (0, deps_js_1.extname)(path);
    }
    let entity = null;
    let body = null;
    if (request.headers.has("If-None-Match") && mtime) {
        [body, entity] = await getEntity(path, mtime, stats, maxbuffer, response);
        const etag = await (0, etag_js_1.calculate)(entity);
        if (etag && (!await (0, etag_js_1.ifNoneMatch)(request.headers.get("If-None-Match"), etag))) {
    ...
    if (request.headers.has("If-Range") && mtime &&
        await (0, range_js_1.ifRange)(request.headers.get("If-Range"), mtime, entity) &&
