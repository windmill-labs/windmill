 ...
 * @deprecated (will be removed in 0.212.0) Use {@link https://deno.land/std/http/cookie.ts} instead.
 */
exports.cookieMapHeadersInitSymbol = Symbol.for("Deno.std.cookieMap.headersInit");
...
class SecureCookieMap extends CookieMapBase {
    ...
    constructor(request, options = {}) {
        ...
        const { keys } = options;
        tslib_1.__classPrivateFieldSet(this, _SecureCookieMap_keyRing, keys, "f");
    }
    ...
    async get(key, options = {}) {
        const signed = options.signed ?? !!tslib_1.__classPrivateFieldGet(this, _SecureCookieMap_keyRing, "f");
        ...
        if (!tslib_1.__classPrivateFieldGet(this, _SecureCookieMap_keyRing, "f")) {
            throw new TypeError("key ring required for signed cookies");
        }
        const index = await tslib_1.__classPrivateFieldGet(this, _SecureCookieMap_keyRing, "f").indexOf(data, digest);
        if (index < 0) {
            await this.delete(nameSig, { path: "/", signed: false });
        }
        else {
            if (index) {
                await this.set(nameSig, await tslib_1.__classPrivateFieldGet(this, _SecureCookieMap_keyRing, "f").sign(data), {
                    signed: false,
                });
            }
            return value;
    ...
    async set(key, value, options = {}) {
        ...
        if (signed) {
            ...
            cookie.value = await tslib_1.__classPrivateFieldGet(this, _SecureCookieMap_keyRing, "f").sign(cookie.toString());
            cookie.name += ".sig";
            pushCookie(headers, cookie);
        }
        resHeaders.delete("set-cookie");
