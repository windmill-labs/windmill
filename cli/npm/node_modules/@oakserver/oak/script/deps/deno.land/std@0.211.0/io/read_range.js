...
async function readRange(r, range) {
    ...
    (0, assert_js_1.assert)(length > 0, "Invalid byte range was passed.");
    ...
    while (length) {
        ...
        (0, assert_js_1.assert)(nread !== null, "Unexpected EOF reach while reading a range.");
        (0, assert_js_1.assert)(nread > 0, "Unexpected read of 0 bytes while reading a range.");
        (0, copy_js_1.copy)(p, result, off);
        off += nread;
        length -= nread;
        (0, assert_js_1.assert)(length >= 0, "Unexpected length remaining after reading range.");
    }
    return result;
}
