...
var parallel_channel = require('./parallel/channel.js');
...
async function run(script, args, options) {
    ...
    const maxWorkers = run.maxWorkers || parallel.default.maxWorkers || await parallel_threads.getMaxParallelism;
    const fn = (options === null || options === void 0 ? void 0 : options.fn) || "default";
    let modId = parallel_module.sanitizeModuleId(script);
    ...
    if (poolRecord) {
        ...
        workerPool.push(poolRecord = {
            getWorker: parallel_threads.createWorker({ parallel: parallel.default, adapter }),
            adapter,
            busy: true,
            lastAccess: Date.now(),
        });
    ...
    if (timeout) {
        runtime.unrefTimer(timeout);
    }
    const handleMessage = async (msg) => {
        var _a, _b;
        if (parallel_channel.isChannelMessage(msg)) {
            await parallel_channel.handleChannelMessage(msg);
        }
        else if (parallel_threads.isCallResponse(msg)) {
            ...
            if (msg.type === "return" || msg.type === "error") {
                if (msg.type === "error") {
                    ...
                    result = { value: parallel_threads.unwrapReturnValue(msg.value) };
                }
                ...
                const value = parallel_threads.unwrapReturnValue(msg.value);
                if (msg.done) {
    ...
    const handleClose = (err, terminated = false) => {
        ...
        if (err) {
            error$1 !== null && error$1 !== void 0 ? error$1 : (error$1 = err);
        }
        if (error$1) {
            if (promise) {
                ...
                if (error$1 instanceof Error) {
                    ...
                    channel.close(new Error(error$1));
                }
                else {
                    // @ts-ignore
                    channel.close(new Error("unknown error", { cause: error$1 }));
                }
            }
        }
        else {
    ...
    if (env.isNode || env.isBun) {
        if (adapter === "child_process") {
            ...
            const { args } = parallel_threads.wrapArgs(req.args, Promise.resolve(worker));
            req.args = args;
            ...
            const { args, transferable, } = parallel_threads.wrapArgs(req.args, Promise.resolve(worker));
            req.args = args;
        ...
        worker.onerror = (ev) => {
            ...
            if (!error$1 && !result) {
                ...
                handleClose((_a = error.fromErrorEvent(ev)) !== null && _a !== void 0 ? _a : new Error("worker exited"), true);
            }
        };
        release = () => {
            worker.onmessage = null;
...
Object.defineProperties(run, {
    workerEntry: {
        set(v) {
            parallel.default.workerEntry = v;
        },
        get() {
            return parallel.default.workerEntry;
        },
    },
});
var run$1 = run;
