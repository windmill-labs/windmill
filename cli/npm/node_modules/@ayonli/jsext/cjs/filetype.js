...
var filetype_constants = require('./filetype/constants.js');
...
function getUTI(type) {
    ...
    for (const [uti, values] of Object.entries(filetype_constants.UTIMap)) {
...
function getMIME(type) {
    ...
    if (!type) {
        ...
        const values = filetype_constants.UTIMap[type] || null;
        return values ? values.find(v => v.includes("/")) : undefined;
    }
    for (const values of Object.values(filetype_constants.UTIMap)) {
        if (values.includes(type)) {
...
function getExtensions(type) {
    ...
    if (type.endsWith("/*")) {
        ...
        return Object.values(filetype_constants.UTIMap)
            .filter(values => values.some(v => v !== type && v.startsWith(_type)))
            .map(values => values.filter(v => v[0] === "."))
            .flat();
    }
    for (const types of Object.values(filetype_constants.UTIMap)) {
