...
const FilenameTooLongError = new error_Exception.default("UStar format does not allow a long file name (length of [file name"
...
function formatHeader(data) {
    ...
    for (const [field, length] of Object.entries(USTarFileHeaderFieldLengths)) {
        const entry = bytes.default(data[field] || "");
...
function createEntry(headerInfo) {
    ...
    const relativePath = (headerInfo.prefix ? headerInfo.prefix + "/" : "")
        + string.stripEnd(headerInfo.name, "/");
    return {
...
class Tarball {
    ...
    constructEntry(relativePath, data, info) {
        ...
        if (typeof data === "string") {
            const _data = bytes.default(data);
            body = reader.toReadableStream([_data]);
            ...
            body = reader.toReadableStream([data]);
            size = data.byteLength;
        }
    ...
    *entries() {
    ...
    static async load(stream, options = {}) {
        ...
        const tarball = new Tarball();
        const reader$1 = stream.getReader();
        ...
        try {
            outer: while (true) {
                const { done, value } = await reader$1.read();
                if (done) {
                    break;
                }
                ...
                while (true) {
                    ...
                    if (writtenBytes === fileSize) {
                        const _entry = {
                            ...entry,
                            header: header,
                            body: reader.toReadableStream(writer),
                        };
