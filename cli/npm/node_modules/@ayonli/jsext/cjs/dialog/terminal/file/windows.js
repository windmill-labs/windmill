...
function htmlAcceptToFileFilter(accept) {
    ...
    return groups.map(group => {
        if (Array.isArray(group)) {
            const patterns = group.map(type => filetype.getExtensions(type).map(t => `*${t}`))
                .flat()
                .join(";");
            ...
            const patterns = filetype.getExtensions(group).map(t => `*${t}`).join(";");
            if (!patterns) {
                return undefined;
            }
            else if (group === "video/*") {
    ...
    if (mode === "file") {
        if (forSave) {
            ...
            return string.dedent `
                ...
                $saveFileDialog.Title = "${dialog_terminal_util.escape(title)}"
                $saveFileDialog.FileName = "${defaultName ? dialog_terminal_util.escape(defaultName) : ""}"
            ...
            return string.dedent `
                ...
                $openFileDialog.Title = "${dialog_terminal_util.escape(title)}"
        ...
        return string.dedent `
            ...
            $folderBrowserDialog.Description = "${dialog_terminal_util.escape(title)}"
...
async function windowsPickFile(title = "", options = {}) {
    const { code, stdout, stderr } = await cli.powershell(createPowerShellScript("file", title, options));
    if (!code) {
...
async function windowsPickFiles(title = "", type = "") {
    const { code, stdout, stderr } = await cli.powershell(createPowerShellScript("files", title, { type }));
    if (!code) {
        const output = stdout.trim();
        return output ? string.lines(stdout.trim()).map(refinePath) : [];
    }
    else {
        throw new Error(stderr);
    }
...
async function windowsPickFolder(title = "") {
    const { code, stdout, stderr } = await cli.powershell(createPowerShellScript("folder", title));
    if (!code) {
...
exports.windowsPickFile = windowsPickFile;
exports.windowsPickFiles = windowsPickFiles;
exports.windowsPickFolder = windowsPickFolder;
