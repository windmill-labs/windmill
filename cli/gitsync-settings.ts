import { Command, colors, log, yamlStringify } from "./deps.ts";
...
function getCurrentSettings(
    ...
    writeMode: "flat" | "override" | "replace",
    overrideKey?: string,
): SyncOptions {
    if (
        writeMode === "override" &&
        ...
        return { ...DEFAULT_SYNC_OPTIONS, ...localConfig };
    }
}
...
function displayChanges(changes: { [key: string]: { from: any; to: any } }): void {
    ...
    try {
        ...
        if (opts.withBackendSettings) {
            try {
                ...
                if (!parsedSettings.include_path || !Array.isArray(parsedSettings.include_path)) {
                    throw new Error("Invalid settings format. Expected include_path array");
                }
                if (!parsedSettings.include_type || !Array.isArray(parsedSettings.include_type)) {
                ...
                if (opts.jsonOutput) {
                    console.log(
                        JSON.stringify({
                            success: false,
                            error: `Failed to parse --with-backend-settings JSON: ${parseError.message}`,
                        }),
                    );
                } else {
                    log.error(
                        colors.red(
                            `Failed to parse --with-backend-settings JSON: ${parseError.message}`,
                        ),
                    );
                }
                return;
        ...
        let writeMode: "flat" | "override" | "replace" = "flat";
        ...
        if (opts.default) {
            ...
            const currentSettings = getCurrentSettings(
                localConfig,
                "flat", // Check against flat mode first
                undefined,
            );
        ...
        if (
            ...
            if (hasConflict && Deno.isatty(Deno.stdin.rid)) {
                // Interactive mode - ask user
                const { Select } = await import("./deps.ts");
                const choice = await Select.prompt({
                    message:
                        "Settings conflict detected. How would you like to proceed?",
                    options: [
                        {
        ...
        if (writeMode === "flat") {
            // Write flat YAML without overrides
            ...
            const currentEffective = getCurrentSettings(localConfig, "flat");
        ...
        if (opts.jsonOutput) {
            ...
            } else if (writeMode === "flat") {
                log.info(colors.gray(`Settings written as flat configuration`));
            } else {
        ...
        if (opts.jsonOutput) {
            ...
            log.error(
                colors.red(
                    `Failed to pull git-sync settings: ${error.message}`,
                ),
            );
        }
    }
...
async function pushGitSyncSettings(
    ...
    try {
        ...
        if (opts.jsonOutput) {
            ...
            log.error(
                colors.red(
                    `Failed to push git-sync settings: ${error.message}`,
                ),
            );
        }
    }
