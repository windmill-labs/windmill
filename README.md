...
| WORKER_EXTERNAL_IP                  | None                             | The address workers report as their egress IP, shown in the workers list and offered for whitelisting when connecting to a database. Set it to skip the lookup against hub.windmill.dev that otherwise resolves it at startup: for deployments that already know the address, or whose egress is blocked and would rather not have every worker process attempt the call. At most 50 characters; a longer value is refused at startup and the lookup runs instead. Setting it is the only way to get an address out of an agent worker whose process is shorter-lived than the lookup, or that runs against a server older than this variable. | Worker                |
