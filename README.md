...
| EXIT_AFTER_N_JOBS                   | None                             | Exit the worker process after it has executed that many jobs (the worker's own init and periodic scripts do not count), so that a supervisor restarts it on a fresh environment. For deployments that isolate executions by process lifetime rather than with nsjail. The worker name is then derived from the hostname instead of being random, so the restarted worker keeps its row in the workers list. Use one worker per process: workers of one process share its environment, so the first to reach the limit shuts the others down too. | Worker                |
