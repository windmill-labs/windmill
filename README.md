...
| WORKER_SUFFIX                       | None                             | Replaces the random part of the worker name, which is what makes a restarted worker keep its row in the workers list. Only needed when several worker processes of the same worker group run on one host, since the name is otherwise derived from the hostname: give each of them a distinct value, as two processes sharing one must never happen. | Worker                |
