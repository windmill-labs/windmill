...
#[cfg(feature = "enterprise")]
use windmill_common::ee_oss::{jobs_waiting_alerts, schedule_overrun_alerts, worker_groups_alerts};
...
pub async fn monitor_db(
    ...
    // Covers the case `jobs_waiting_alerts` structurally cannot: its predicate
    // carries `running = false`, so a schedule whose run overruns is invisible to it.
    let schedule_overrun_alerts_f = async {
        #[cfg(feature = "enterprise")]
        if server_mode && iteration.is_some() && iteration.as_ref().unwrap().should_run(30) {
            if let Some(db) = conn.as_sql() {
                schedule_overrun_alerts(&db).await;
            }
        }
    };
