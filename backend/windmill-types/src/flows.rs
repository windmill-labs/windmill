...
pub struct ListableFlow {
    ...
    /// exists). Distinct from `path` for draft-only rows whose URL path
    /// is the autogenerated `u/{user}/draft_{uuid}` while the user may
    /// have set a meaningful path like `u/{user}/my_flow` — the home
    /// list prefers this for the displayed title.
    #[sqlx(default)]
    #[serde(skip_serializing_if = "Option::is_none")]
