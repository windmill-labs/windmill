...
pub struct ListableScript {
    ...
    /// autogenerated `u/{user}/draft_{uuid}`. Every editor binds the Path widget to
    /// the content's own `path`, so it's sourced from the draft JSON's `value.path`
    /// when it differs from the storage path. `None` = unchanged.
    #[sqlx(default)]
    #[serde(skip_serializing_if = "Option::is_none")]
