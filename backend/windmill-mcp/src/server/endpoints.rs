...
#[cfg(test)]
mod tests {
    use super::{endpoint_tool_to_mcp_tool, EndpointTool};
    use serde_json::json;
    use std::borrow::Cow;

    #[test]
    fn closes_top_level_args_and_keeps_opaque_nested_objects_open() {
        let tool = EndpointTool {
            name: Cow::Borrowed("updateApp"),
            description: Cow::Borrowed("update app"),
            instructions: Cow::Borrowed(""),
            path: Cow::Borrowed("/w/{workspace}/apps/update/{path}"),
            method: Cow::Borrowed("POST"),
            path_params_schema: Some(json!({
                "type": "object",
                "properties": {
                    "path__path": { "type": "string" }
                },
                "required": ["path__path"]
            })),
            ...
            body_schema: Some(json!({
                ...
                "properties": {
                    ...
                    "value": { "type": "object" },
                    "policy": { "type": "object" },
                    "deployment_message": { "type": "string" },
                    "path__body": { "type": "string" }
                }
            })),
            path_field_renames: Some(json!({ "path__path": "path" })),
            query_field_renames: None,
            body_field_renames: Some(json!({ "path__body": "path" })),
        };

        let mcp_tool = endpoint_tool_to_mcp_tool(&tool);
        ...
        assert_eq!(input_schema["required"], json!(["path__path"]));
        assert_eq!(input_schema["properties"]["value"]["properties"], json!({}));
        assert_eq!(
            input_schema["properties"]["value"]["additionalProperties"],
            json!(true)
        );
        assert_eq!(
            input_schema["properties"]["policy"]["properties"],
            json!({})
        );
        assert_eq!(
            input_schema["properties"]["policy"]["additionalProperties"],
            json!(true)
        );
    }
}
