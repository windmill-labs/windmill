...
async fn list_flows(
        ...
        && !lq.show_archived.unwrap_or(false)
    {
        ...
        for row in draft_only_rows {
            ...
            // The user may have typed a path in the editor different
            // from the URL the draft is keyed by — surface it as
            // `draft_path` so the home list can show the meaningful
            // name instead of the autogenerated `draft_{uuid}` path.
            // Only set when it actually differs from `row.path` —
            // saves the frontend a redundant string and keeps the
            // common case (path unchanged) silent in the wire.
            let draft_path = v
                .get("path")
                .and_then(|s| s.as_str())
