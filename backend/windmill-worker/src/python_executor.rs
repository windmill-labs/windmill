...
impl PyV {
    pub(crate) async fn resolve(
        ...
        if !valid.is_empty() {
            ...
            let gravity_matcher =
                VersionSpecifier::from_version(pep440_rs::Operator::EqualStar, (*gv).clone())
                    .map_err(|e| {
  ...
  All versions: \n{}
    ...
    pub(crate) async fn gravitational_version(
        ...
        let mut err = None;
        let pyv = match INSTANCE_PYTHON_VERSION.read().await.clone() {
            Some(v) => Version::from_str(&v).unwrap_or_else(|_| {
    ...
    pub async fn list_available_python_versions() -> anyhow::Result<Vec<Self>> {
        ...
        if output.status.success() {
            ...
            let list = serde_json::from_str::<Vec<Map<String, Value>>>(&res)?
                .into_iter()
                .filter_map(|e| {
                    // TODO: Refactor
                    if e.get("implementation").and_then(Value::as_str) == Some("pypy") {
                        ...
                        Some(
                            e.get("version")
                                ...
                                .map(PyV::from)
                                .ok_or(error::Error::internal_err("version is None")),
                        )
                    }
                })
...
mod tests {
    ...
    fn pyv(value: &str) -> PyV {
        Version::from_str(value).unwrap().into()
    }
    ...
    async fn test_python_resolution_6() {
        assert_resolution(
            "1.1.1",
            vec![],
            vec![pyv("2.2.2"), pyv("2.2.1"), pyv("2.2.0")],
            pyv("2.2.2"),
        )
        .await;
    }
