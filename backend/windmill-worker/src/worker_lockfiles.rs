...
pub async fn handle_dependency_job(
    ...
    // Extract dependency_tree from job args (path -> hash mapping for temp storage)
    let dependency_tree: Option<HashMap<String, String>> = job
        .args
        .as_ref()
        .and_then(|x| x.get("dependency_tree"))
