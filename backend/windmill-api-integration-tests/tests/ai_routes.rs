...
/// Start a mock AI API that 307-redirects every request to an `/internal`
/// route. `internal_hit` flips to true only if a client actually follows the
/// redirect and reaches `/internal` — simulating a redirect that points the
/// server at an internal/private endpoint.
async fn start_mock_redirecting_ai_api(internal_hit: Arc<AtomicBool>) -> u16 {
    use axum::{
    ...
    let app = Router::new()
        .route(
            "/internal",
            any(move || {
                let internal_hit = internal_hit.clone();
                async move {
                    internal_hit.store(true, Ordering::SeqCst);
                    Json(json!({
        ...
        // 307 preserves method + body, the worst case for SSRF.
        .fallback(any(|| async {
            Redirect::temporary("/internal").into_response()
        }));
...
/// Regression test: the AI proxy must not follow HTTP redirects. The SSRF check
/// only validates the configured `base_url`; if redirects were followed, a
/// public base_url could 3xx the server into a private/internal address. Here
/// the mock upstream redirects to its own `/internal` route — the windmill
/// server must return the 3xx without ever reaching `/internal`.
#[sqlx::test(migrations = "../migrations", fixtures("base"))]
async fn test_ai_proxy_does_not_follow_redirects(db: Pool<Postgres>) -> anyhow::Result<()> {
    initialize_tracing().await;
    // Allow the mock AI API on 127.0.0.1 to pass the SSRF check on base_url.
    std::env::set_var("ALLOW_PRIVATE_AI_BASE_URLS", "true");
    let server = ApiServer::start(db.clone()).await?;
    let port = server.addr.port();

    let internal_hit = Arc::new(AtomicBool::new(false));
    let mock_port = start_mock_redirecting_ai_api(internal_hit.clone()).await;
    ...
    let resp = authed(
        client()
            ...
            .json(&json!({
                "path": "f/ai/openai_redirect",
                "resource_type": "openai",
                "value": { "api_key": "test-key", "base_url": mock_url }
            })),
    )
    .send()
    .await?;
    assert_2xx(
        resp.status().as_u16(),
        &resp.text().await?,
        "create redirecting openai resource",
    );

    // Use a client that does NOT follow redirects, so following the upstream
    // 3xx is attributable to the windmill server, not to this test client.
    ...
    let resp = authed(
        no_follow
            ...
            .header("X-Resource-Path", "f/ai/openai_redirect")
            .json(&json!({
                "model": "gpt-4",
    ...
    assert!(
        ...
        "AI proxy followed a redirect to the internal endpoint (SSRF); status {status}: {body}",
    );
    assert!(
        (300..400).contains(&status),
        "expected the proxy to surface the upstream 3xx without following it, got {status}: {body}",
    );
