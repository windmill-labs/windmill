...
/// Regression test for WIN-1978: an unscoped token whose `owner` field
/// references a `u/<username>`, `g/<groupname>`, or unprefixed value that
/// is not present in the target workspace must not authenticate as an
/// operator (or as anything else). Before the fix, the auth fallback at
/// `windmill-api-auth/src/auth.rs` granted `(is_admin=false, is_operator=true)`
/// when the `usr` lookup returned no row, letting an attacker who could
/// mutate the `token` table (e.g. via a SQL preview against the metadata
/// DB exposed in the official Docker Compose) cross workspace boundaries.
#[ignore]
#[cfg(feature = "deno_core")]
#[sqlx::test(migrations = "../migrations", fixtures("base", "permissions_test"))]
async fn test_unscoped_token_missing_owner_rejects_cross_workspace(
    db: Pool<Postgres>,
) -> anyhow::Result<()> {
    initialize_tracing().await;
    let server = ApiServer::start(db.clone()).await?;
    ...
    // Insert three unscoped (workspace_id = NULL) tokens with forged owners.
    // Each represents an attacker who modified their own token's owner field.
    let forged_owners = [
        ("FORGED_MISSING_USER_X", "u/candidate1_missing_user"),
        ("FORGED_MISSING_GROUP_", "g/candidate1_missing_group"),
        ("FORGED_NO_PREFIX_TOK_", "raw_no_prefix"),
    ];

    for (token, owner) in forged_owners.iter() {
        let token_hash = windmill_common::utils::calculate_hash(token);
        let token_prefix = &token[..10.min(token.len())];
        sqlx::query!(
            "INSERT INTO token (token_hash, token_prefix, email, label, super_admin, owner, workspace_id)
             VALUES ($1, $2, 'attacker@example.com', 'forged', false, $3, NULL)",
            token_hash,
            token_prefix,
            owner,
        )
    ...
    // Each forged token must be rejected when used against the target workspace.
    for (token, owner) in forged_owners.iter() {
        let client = create_client_for_user(port, token).await;
        let resp = client
        ...
        assert!(
            ...
            "Forged unscoped token with owner '{owner}' should be rejected against test-workspace; got {}",
            resp.status()
        );
    }
