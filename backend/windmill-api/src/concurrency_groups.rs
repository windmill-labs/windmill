...
async fn get_concurrent_intervals(
    ...
    let mut sqlb_q = SqlBuilder::select_from("v2_queue")
        .fields(UnifiedJob::queued_job_fields())
        .order_by("created_at", lq.order_desc.unwrap_or(true))
        .limit(row_limit)
        .clone();
    let mut sqlb_c = SqlBuilder::select_from("v2_completed_job")
        .fields(UnifiedJob::completed_job_fields())
        .order_by("started_at", lq.order_desc.unwrap_or(true))
        .limit(row_limit)
        .clone();
    let mut sqlb_q_user = SqlBuilder::select_from("v2_queue")
        .fields(&["id"])
        .order_by("created_at", lq.order_desc.unwrap_or(true))
        .limit(row_limit)
        .clone();
    let mut sqlb_c_user = SqlBuilder::select_from("v2_completed_job")
        .fields(&["id"])
        .order_by("started_at", lq.order_desc.unwrap_or(true))
        .limit(row_limit)
        .clone();
