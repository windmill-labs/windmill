...
/// Get a cached AWS Secrets Manager backend or create a new one if settings changed
#[cfg(all(feature = "private", feature = "enterprise"))]
async fn get_or_create_aws_sm_backend(
    ...
    let backend: Arc<dyn SecretBackend> = Arc::new(AwsSecretsManagerBackend::new(settings.clone()));
