...
async fn create_script(
    ...
    sqlx::query!(
        "INSERT INTO script (workspace_id, hash, path, parent_hashes, summary, description, \
         ...
         delete_after_use, timeout, concurrency_key, visible_to_runner_only, no_main_func, object_store_sha256) \
...
async fn get_script_by_path_w_draft(
    ...
    let script_o = sqlx::query_as::<_, ScriptWDraft>(
        "SELECT hash, script.path, summary, description, content, language, kind, tag, schema, draft_only, envs, concurrent_limit, concurrency_time_window_s, cache_ttl, ws_error_handler_muted, draft.value as draft, dedicated_worker, priority, restart_unless_cancelled, delete_after_use, timeout, concurrency_key, visible_to_runner_only, no_main_func, object_store_sha256 FROM script LEFT JOIN draft ON 
