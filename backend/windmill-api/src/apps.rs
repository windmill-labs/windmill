...
pub struct ListableApp {
    ...
    pub is_draft: bool,
    /// Path the user typed in the editor for this draft (when one
    /// exists and differs from `path`). For draft-only rows whose URL
    /// path is the autogenerated `u/{user}/draft_{uuid}`, the user may
    /// have set a meaningful path — the home list prefers this when
    /// rendering. `None` when the draft's path matches the URL path
    /// (the common case).
    #[sqlx(default)]
    #[serde(skip_serializing_if = "Option::is_none")]
...
async fn list_apps(
        ...
        && !lq.starred_only.unwrap_or(false)
    {
        ...
        for row in draft_only_rows {
            ...
            // See `list_flows`: surface the user-typed path when it
            // differs from the URL path so the home list can render the
            // meaningful name instead of the autogenerated draft slot.
            let draft_path = v
                .get("path")
                .and_then(|s| s.as_str())
