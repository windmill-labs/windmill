...
Creates a new version of an existing script when called with the same path and the current `parent_hash`"),
    ...
    EndpointTool {
        ...
        instructions: Cow::Borrowed("Use this to change a raw (full-code) app — an app whose `raw_app` field is true. Send the whole `value` (`files`, `runnables`, `data`), not a patch: read the current one with getAppByPath first and edit it. The sources are compiled on a worker by the same build the editor and the CLI run, so a compile error comes back as the error of this call. Compiling runs the app's own dependencies, so the token needs `jobs:run` as well as `apps:write` (selecting this tool grants it). Low-code apps use updateApp instead."),
