...
// --- Sandbox / general-purpose object store helpers ---

/// Download bytes from an object store. Returns `None` if the key doesn't exist or is empty.
#[cfg(feature = "parquet")]
    ...
    match store.get(&path).await {
        Ok(result) => {
            let bytes = result.bytes().await.map_err(|e| {
                error::Error::ExecutionErr(format!("Failed to read S3 bytes: {e}"))
            })?;
            if bytes.is_empty() {
                Ok(None)
            } else {
        ...
        Err(e) => Err(error::Error::ExecutionErr(format!("S3 fetch failed: {e}"))),
    }
}

/// Upload bytes to an object store.
#[cfg(feature = "parquet")]
pub async fn put_bytes_to_store(
    ...
    store
        .put(&path, bytes.into())
        .await
        .map_err(|e| error::Error::ExecutionErr(format!("S3 upload failed: {e}")))?;
    Ok(())
}

/// Build an object store client from a workspace's large_file_storage settings.
#[cfg(feature = "parquet")]
pub async fn get_workspace_object_store(
    ...
    if let LargeFileStorage::FilesystemStorage(ref fs) = lfs {
        return build_filesystem_client(&fs.root_path);
    }
    ...
    let osr = lfs_to_object_store_resource(&lfs, resource_value)?;
    build_object_store_client(&osr).await
}

/// Delete an object from a workspace's object store.
#[cfg(feature = "parquet")]
