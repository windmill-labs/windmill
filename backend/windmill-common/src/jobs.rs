...
#[derive(Serialize, Debug)]
#[serde(untagged)]
pub enum FormattedResultRef<'a> {
    RawValueRef(Option<&'a RawValue>),
    Vec(Vec<Box<RawValue>>),
}
...
pub fn format_result_ref<'a>(
    language: Option<&ScriptLang>,
    flow_status: Option<&'a RawValue>,
    result: Option<&'a RawValue>,
) -> FormattedResultRef<'a> {
    if language == Some(&ScriptLang::Postgresql) {
        if let Some(flow_status) = flow_status {
            let flow_status = serde_json::from_str::<FlowStatusWithMetadataOnly>(flow_status.get());
            if let Ok(flow_status) = flow_status {
                if let Some(result) = result {
                    let rows = serde_json::from_str::<Vec<Box<RawValue>>>(result.get()).ok();
                    if let Some(mut rows) = rows {
                        if let Some(first_row) = rows.get(0) {
                            let first_row = serde_json::from_str::<HashMap<String, Box<RawValue>>>(
                                first_row.get(),
                            );
                            if let Ok(first_row) = first_row {
                                let mut new_first_row = IndexMap::new();
                                for col in flow_status.metadata.column_order {
                                ...
                                return FormattedResultRef::Vec(rows);
                            }
                        }
                    }
                }
    ...
    FormattedResultRef::RawValueRef(result)
}
            ...
            if let Ok(flow_status) = flow_status {
                if let Some(result) = result {
                    ...
                    if let Some(mut rows) = rows {
                        ...
                        return FormattedResult::Vec(rows);
                    }

                    return FormattedResult::RawValue(Some(result));
                }
            }
        }
    }
