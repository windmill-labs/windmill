...
impl Hash for FlowData {
    fn hash<H: std::hash::Hasher>(&self, state: &mut H) {
        self.raw_flow.get().hash(state);
    }
}
        ...
        NODES
            .get_or_insert_async(
                ...
                |(script, flow)| match flow {
                    ...
                    None => RawData::Script(Arc::new(script)),
                },
                async {
                    sqlx::query!(
                        "SELECT \
...
pub mod job {
    ...
    async fn fetch_raw_flow(
        ...
        lazy_static! {
            ...
            static ref CACHE: Cache<Uuid, Arc<FlowData>> = Cache::new(50);
        }
    ...
    pub async fn fetch_raw_script(
        ...
        lazy_static! {
            /// Very small in-memory cache for "preview" jobs raw data.
            static ref CACHE: Cache<Uuid, Arc<ScriptData>> = Cache::new(50);
        }
        ...
        CACHE
            .get_or_insert_async(job, async {
                ...
                .map(|(lock, code)| ScriptData::from_raw(lock, code))
                .map(Arc::new)
            })
            .await
    }
    ...
    pub async fn fetch_script(
        ...
        match (kind, hash.map(|ScriptHash(id)| id)) {
            ...
            (Preview | Dependencies, None) => fetch_raw_script(e, job, raw_lock, raw_code).await,
    ...
    pub async fn fetch_flow(
        ...
        kind: JobKind,
        hash: Option<ScriptHash>,
        // original raw value from `queue` or `completed_job` tables:
        // kept for backward compatibility.
        raw_flow: Option<Json<Box<RawValue>>>,
    ) -> error::Result<Arc<FlowData>> {
        use JobKind::*;
        match (kind, hash.map(|ScriptHash(id)| id)) {
            (Flow | FlowPreview, None) => fetch_raw_flow(e, job, raw_flow).await,
