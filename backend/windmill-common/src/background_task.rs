...
/// Arbitrary read-modify-write on the stored value under the Postgres
/// advisory lock `lock_id`, without touching `running`/`owner`/`started_at`.
///
/// Used for non-leased coordination state (e.g. the list of staggered
/// graceful-restart schedules) where concurrent writers just need a
/// serialization point, not a single-owner lease.
pub async fn rmw_value<T, E>(
    db: &Pool<Postgres>,
    name: &str,
    lock_id: i64,
    f: impl FnOnce(Option<serde_json::Value>) -> std::result::Result<(serde_json::Value, T), E>,
) -> std::result::Result<T, anyhow::Error>
where
    E: Into<anyhow::Error>,
{
    use anyhow::Context as _;
    let mut tx = db.begin().await.context("begin background_task rmw tx")?;

    sqlx::query("SELECT pg_advisory_xact_lock($1)")
        .bind(lock_id)
        .execute(&mut *tx)
        .await
        .context("acquire background_task rmw lock")?;
    ...
    let existing: Option<serde_json::Value> =
        sqlx::query_scalar("SELECT value FROM background_task_state WHERE name = $1")
            ...
            .await
            .context("read background_task value")?;

    let (new_value, ret) = f(existing).map_err(Into::into)?;

    sqlx::query(
        "INSERT INTO background_task_state (name, value, running, updated_at)
    ...
    .execute(&mut *tx)
    .await
    .context("write background_task value")?;

    tx.commit().await.context("commit background_task rmw tx")?;
