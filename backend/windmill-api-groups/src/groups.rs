...
#[cfg(feature = "private")]
async fn workspaces_referencing_instance_groups(
    groups: &[String],
    tx: &mut Transaction<'_, Postgres>,
) -> Result<Vec<String>> {
    if groups.is_empty() {
...
#[cfg(feature = "private")]
async fn reprocess_workspace_instance_groups(
    workspaces: &[String],
    tx: &mut Transaction<'_, Postgres>,
    authed: &ApiAuthed,
) -> Result<()> {
...
async fn overwrite_igroups(
    ...
    // Settings before usr, matching workspaces::edit_instance_groups — the reverse order
    // deadlocks against a concurrent edit. Must precede the deletes below, which remove the
    // rows membership is resolved from.
    #[cfg(feature = "private")]
    {
        use windmill_api_workspaces::workspaces_ee::cleanup_removed_instance_groups;
    ...
    // After the re-insert, so membership reflects the imported payload rather than the
    // wiped-out state.
    #[cfg(feature = "private")]
    reprocess_workspace_instance_groups(&affected_workspaces, &mut tx, &authed).await?;
