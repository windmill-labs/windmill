...
fn map_quickjs_error(err: rquickjs::CaughtError<'_>, memory_limit: usize) -> anyhow::Error {
    let is_oom = match &err {
        // QuickJS's OOM InternalError carries exactly "out of memory"; match it
        // exactly rather than by substring so an unrelated user error whose text
        // happens to mention memory is not swept into the OOM bucket.
        rquickjs::CaughtError::Exception(e) => e.message().as_deref() == Some("out of memory"),
...
mod tests {
    ...
    async fn test_eval_user_throw_not_reported_as_oom() {
        ...
        // the memory limit — only OOM's null/undefined throw is. The last case
        // guards the exact-match on the Exception branch: a message that merely
        // contains the OOM text is not itself an OOM.
        for expr in [
            "(() => { throw 'boom' })()",
