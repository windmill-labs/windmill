...
//!
//! `folders/list` and `folders/listnames` must narrow their results to the paths
//! a scoped token may read, while leaving unscoped callers' listings untouched.
...
async fn test_folder_list_scope_filtering(db: Pool<Postgres>) -> anyhow::Result<()> {
    ...
    assert_eq!(
        list_folder_names(port, "SECRET_TOKEN", "").await?,
        vec!["alpha", "zeta"]
    );
