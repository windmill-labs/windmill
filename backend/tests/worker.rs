...
/// Regression for the hourly-`{partition}` footgun: a partitioned materialize's
/// `{partition}` token substitutes to the partition IDENTITY string, and the
/// grain-agnostic filter idiom the scaffold/AI teach is
/// `WHERE strftime(<ts_col>, '<fmt>') = {partition}`. That only works if the
/// `<fmt>` DuckDB's `strftime` uses matches the format the EE resolver builds
/// the identity from (`partition_ee::default_format`) — otherwise the filter
/// silently returns no rows. This proves, end-to-end through the real DuckDB
/// FFI, that (1) `strftime(ts, fmt)` equals the chrono-computed identity for
/// every grain, (2) the hourly filter buckets the WHOLE hour (not just the
/// boundary instant), and (3) the naive `TIMESTAMP {partition}` a user would
/// reach for after learning the daily pattern actually errors for hourly.
#[cfg(feature = "duckdb")]
#[sqlx::test(fixtures("base"))]
async fn test_duckdb_partition_token_strftime_idiom(db: Pool<Postgres>) -> anyhow::Result<()> {
    ...
    // Format strings MUST mirror `windmill-common/src/partition_ee.rs`
    // `default_format` — the resolver stamps the stored identity with these, and
    // the scaffolded filter compares against it via DuckDB `strftime`.
    let instant = Utc.with_ymd_and_hms(2026, 7, 5, 23, 10, 0).unwrap();
    let hourly = instant.format("%Y-%m-%dT%H").to_string(); // 2026-07-05T23
    let weekly = instant.format("%G-W%V").to_string(); // 2026-W27
    let monthly = instant.format("%Y-%m").to_string(); // 2026-07
    let daily = instant.format("%Y-%m-%d").to_string(); // 2026-07-05

    let run_scalar = |content: String| {
        let db = db.clone();
        async move {
            let flow: FlowValue = serde_json::from_value(serde_json::json!({
                "modules": [{
                    "value": { "type": "rawscript", "language": "duckdb", "content": content },
                }],
            }))
            .unwrap();
            RunJob::from(JobPayload::RawFlow { value: flow, path: None, restarted_from: None })
    ...
    // (1) chrono identity == DuckDB strftime for every grain, and (2) the hourly
    // filter matches both 23:10 and 23:55 (the whole hour) — not just an exact
    // boundary instant, which is all `= TIMESTAMP {partition}` could ever match.
    let ts = "TIMESTAMP '2026-07-05 23:10:00'";
    let idiom = format!(
        "-- result_collection=last_statement_first_row_scalar\n\
         WITH t(ts) AS (VALUES\n\
           (TIMESTAMP '2026-07-05 23:10:00'),\n\
           (TIMESTAMP '2026-07-05 23:55:00'),\n\
           (TIMESTAMP '2026-07-05 22:30:00'),\n\
           (TIMESTAMP '2026-07-06 00:05:00'))\n\
         SELECT strftime({ts}, '%Y-%m-%dT%H')\n\
           || '|' || strftime({ts}, '%G-W%V')\n\
           || '|' || strftime({ts}, '%Y-%m')\n\
           || '|' || strftime({ts}, '%Y-%m-%d')\n\
           || '|' || (SELECT count(*) FROM t \
                      WHERE strftime(t.ts, '%Y-%m-%dT%H') = '{hourly}')::VARCHAR;"
    );
    let got = run_scalar(idiom).await;
    assert!(
        got.success,
        "scaffolded strftime idiom failed to run: {:?}",
        got.result
    );
    assert_eq!(
        got.json_result().unwrap(),
        serde_json::json!(format!("{hourly}|{weekly}|{monthly}|{daily}|2")),
        "strftime format drifted from the resolver identity, or the hourly slice \
         did not bucket the whole hour"
    );

    // (3) The naive comparison a user copies from the daily pattern — the
    // identity string is not a valid DuckDB TIMESTAMP literal for hourly, so the
    // job must fail loudly (the footgun this fix documents around).
    let naive = format!("SELECT TIMESTAMP '{hourly}';");
    let footgun = run_scalar(naive).await;
    assert!(
        !footgun.success,
        "expected `TIMESTAMP '{hourly}'` to error (invalid timestamp literal), but the job succeeded: {:?}",
        footgun.result
    );
