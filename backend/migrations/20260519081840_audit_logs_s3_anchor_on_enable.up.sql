...
-- store export cursor *in the enabling transaction*. Capturing
-- txid_snapshot_xmin here (rather than the settings row's own xmin, or a later
-- async/first-tick read) is the only boundary that is correct for the common
-- case where audit_log() runs in a caller transaction that acquired its xid
-- before the enable: such a transaction is in-flight at this snapshot, so its
-- xid >= the snapshot xmin and its post-enable audit rows are still exported.
...
-- the trigger to the `store_audit_logs_s3` row, so the recursive INSERT (a
-- different name) does not re-enter.
